...
/**
 * AI course recommendations from catalog
 */
async function getCourseRecommendations({ stage, goals = [], preference, experience, courses = [] }) {
    ...
    const systemPrompt = `You are Mamasafe's maternal education AI. Recommend courses from the provided catalog only.
...
${JSON.stringify(catalog, null, 2)}`;
    ...
    try {
        const text = await chat(systemPrompt, userPrompt, 800);
...
/**
 * Generate lesson content for a course module
 */
async function getModuleLesson({ courseTitle, moduleTitle, moduleIndex, totalModules, stage, goals = [] }) {
    const systemPrompt = `You are Mamasafe's expert maternal health educator. Create engaging lesson content for mothers.
...
Write a complete mini-lesson the user can read in 5-8 minutes.`;
    ...
    try {
        console.log('Calling Groq AI for module:', moduleTitle);
        const text = await chat(systemPrompt, userPrompt, 1500);
        console.log('Groq AI response received for module:', moduleTitle);
        const parsed = parseJsonFromText(text);
        ...
        return {
            ...
            lesson: {
                ...
                keyTakeaways: [
                    ...
                    'Track progress in your Mamasafe dashboard'
                ],
...
/**
 * Expert Q&A for courses community
 */
async function askCourseExpert({ question, stage, courseContext }) {
    const systemPrompt = `You are a certified maternal health educator on Mamasafe. Answer parenting and pregnancy questions clearly and supportively.
...
${courseContext ? `Course context: ${courseContext}` : ''}`;
    ...
    try {
        console.log('Groq AI chatbot call for question:', question);
        const answer = await chat(systemPrompt, userPrompt, 600);
        console.log('Groq AI chatbot response received');
        return { success: true, answer, timestamp: new Date().toISOString() };
    } catch (error) {
        console.error('Groq AI chatbot error:', error.message);
        console.error('Full error:', error);
...
/**
 * Community discussion starter / group insight
 */
async function getCommunityInsight({ topic, stage }) {
    const systemPrompt = `You are facilitating Mamasafe Mother's Circle. Give a warm welcome and 2 discussion prompts for mothers. Under 150 words.`;

    const userPrompt = `Topic: ${topic || 'general support'}
Stage: ${stage || 'all stages'}`;

    try {
        const message = await chat(systemPrompt, userPrompt, 400);
