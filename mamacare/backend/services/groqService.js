...
const SAFETY_NOTE = 'MamaCare provides health education and does not replace advice from a qualified healthcare professional.';
...
function buildSystemPrompt({ emergency = false } = {}) {
    return [
        'You are MamaCare, a pregnancy health education assistant powered by Llama 3.3 70B via Groq.',
...
function normalizeGroqRiskAssessment(raw = {}, { model, emergency = false } = {}) {
    ...
    return {
        ...
        riskLevel,
        rating: riskLevel,
        confidenceScore: normalizeScore(raw.confidenceScore ?? raw.confidence ?? raw.accuracy, emergency ? 0.85 : 0.65),
...
function buildRiskAssessmentPrompt({ question, week, symptoms, symptomAnalysis, emergency }) {
    return [
        ...
        '',
        'Safety baseline from MamaCare rules:',
...
async function assessPregnancyRiskWithGroq({
    ...
    const content = await chatWithGroq(
        [
            'You are MamaCare pregnancy risk assessment AI using Llama 3.3 70B via Groq.',
