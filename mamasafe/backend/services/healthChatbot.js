...
const { processWithGroq } = require('./groqChatbot');
...
/**
 * Process health query using Llama 3.3 70B via Groq with local knowledge fallback
...
async function processHealthQuery(userMessage, userContext = {}, chatHistory = []) {
    console.log('Processing health query with Groq/Llama 3.3 70B...');

    try {
        console.log('Using Groq...');
        const aiResponse = await processWithGroq(userMessage, userContext, chatHistory);
