(() => {
  ...
  const formatRagBriefing = (data) => {
    const answer = data.reply || data.answer || data.response || "";
    const matches = Array.isArray(data.matches) ? data.matches : [];
    const sources = matches.slice(0, 5).map((match) => {
      const label = [match.collection, match.title || match.dataset || match.sourceOrganization]
        .filter(Boolean)
        .join(" - ");
      return label ? `- ${label}` : "";
    }).filter(Boolean);
    ...
    return [
      answer,
      sources.length ? "\nMongoDB records used:\n" + sources.join("\n") : "",
      "\nPipeline: MongoDB dataset -> dataset-only answer"
    ].filter(Boolean).join("\n");
  };

  const callPregnancyRagEndpoint = async ({ week, symptoms, concerns }) => {
    const question = [
      concerns || `Create a practical pregnancy briefing for week ${week}.`,
      symptoms ? `Symptoms or notes: ${symptoms}.` : "",
      "Include baby development, mother body changes, priority actions, and warning signs that need a clinician."
    ].filter(Boolean).join(" ");

    const res = await fetch(`${getBackendOrigin()}/api/pregnancy-rag/ask`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ question, week, symptoms })
    });
    const data = await readResponseBody(res);
    if (!res.ok || data.success === false) {
      throw new Error(data.error || data.details || `Request failed (${res.status})`);
    }
    return formatRagBriefing(data);
  };
  ...
  const generateAiBriefing = async (week, symptoms, concerns) => {
    setAiOutput("Searching MongoDB pregnancy dataset and building a dataset-only briefing...", "loading");
    try {
      const text = await callPregnancyRagEndpoint({ week, symptoms, concerns });
      const finalText = text || offlineBrief(week, symptoms, concerns);
      setAiOutput(finalText, "ready");
      return finalText;
    } catch (e) {
      const msg = e.message || "backend unavailable";
      const fallback = `${offlineBrief(week, symptoms, concerns)}\n\nMongoDB RAG status: ${msg}`;
  ...
  const renderFocus = (state) => {
    ...
    if (brief) {
      brief.textContent = "Generate an instant briefing from the MongoDB pregnancy dataset.";
    }
  };
      ...
      aiForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          if (submit) {
            ...
            submit.textContent = "Generate Dataset Briefing";
          }
        }
      });
    }
