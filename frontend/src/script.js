...
function showFullMilestones() {
    ...
    babyMilestonesByMonth.forEach((data, index) => {
        ...
        timelineHTML += `
            <div style="display: flex; gap: 15px; padding: 20px; background: ${isCurrent ? 'linear-gradient(135deg, #e8eaf6 0%, #fbc2eb 100%)' : 'white'}; 
                         border-radius: 12px; border-left: 4px solid ${isPast ? '#00d4aa' : isCurrent ? '#ff6b9d' : '#e0e0e0'};
...
let familyData = {
    ...
    goals: [
        { id: 1, title: 'Weekly Family Dinners', progress: 85, color: '#00b894', target: 7, current: 6, unit: 'dinners' },
        { id: 2, title: 'Savings Target', progress: 60, color: '#ff9800', target: 1000, current: 600, unit: '$' },
        { id: 3, title: 'Exercise Minutes', progress: 72, color: '#00b894', target: 300, current: 216, unit: 'minutes' },
        { id: 4, title: 'Family Reading Time', progress: 45, color: '#667eea', target: 420, current: 189, unit: 'minutes' }
    ],
    insights: [
...
function loadFamilyMembers() {
    ...
    familyData.members.forEach(member => {
        const roleColor = member.role === 'Parent' ? '#667eea' : '#ff6b9d';
        const statusColor = member.status === 'active' ? '#00b894' : '#9e9e9e';
        ...
        html += `
            <div style="display: flex; align-items: center; gap: 12px; padding: 15px; background: white; border-radius: 12px; border: 2px solid #f0f0f0; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.05);"
                ...
                <div style="width: 50px; height: 50px; background: linear-gradient(135deg, ${roleColor} 0%, ${member.role === 'Parent' ? '#0f2a56' : '#ff6b9d'} 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 24px; box-shadow: 0 3px 10px rgba(0,0,0,0.2);">
                    ${member.avatar}
                </div>
                <div style="flex: 1;">
                    <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 2px;">${member.name}</div>
    ...
    familyData.schedule.forEach(item => {
        ...
        html += `
            <div style="display: flex; align-items: center; gap: 12px; padding: 12px; margin-bottom: 8px; background: ${item.completed ? '#f8f9fa' : 'white'}; border-radius: 8px; border-left: 4px solid ${priorityColor}; cursor: pointer; transition: all 0.2s ease; position: relative; overflow: hidden;"
                ...
                <div style="flex: 1;">
                    <div style="font-size: 14px; color: ${item.completed ? '#999' : '#0f2a56'}; font-weight: 600; margin-bottom: 2px;">${item.activity}</div>
                    <div style="font-size: 12px; color: #666;">
                        <span style="color: ${item.completed ? '#00b894' : (isPast ? '#ff6b9d' : '#667eea')}">${item.participants.join(', ')}</span>
                        ${isNow ? '<span style="color: #ff6b9d; font-weight: 600; margin-left: 8px;"> NOW</span>' : ''}
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    ${item.completed ? 
                        `<div style="width: 24px; height: 24px; background: #00b894; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 14px;"> check</div>` :
                        `<div style="width: 24px; height: 24px; background: ${isPast ? '#ff6b9d' : '#e0e0e0'}; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 14px;">${isPast ? '!' : 'o'}</div>`
                    }
                </div>
            </div>
        `;
...
function loadFamilyGoals() {
    ...
    familyData.goals.forEach(goal => {
        const progressColor = goal.progress >= 80 ? '#00b894' : goal.progress >= 50 ? '#ff9800' : '#ff4757';
        ...
        html += `
            <div style="background: white; padding: 20px; border-radius: 12px; border-left: 4px solid ${goal.color}; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.05);"
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <div style="font-weight: 700; color: #0f2a56; font-size: 16px;">${goal.title}</div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="font-size: 12px; color: ${remaining > 0 ? '#ff9800' : '#00b894'}; font-weight: 600;">
                        ${remainingText}
                    </div>
                </div>
            </div>
...
function addFamilyGoal() {
    ...
    const colors = ['#00b894', '#ff9800', '#00b894', '#667eea', '#ff4757'];
...
function addFloatingAnimation() {
    ...
    style.textContent = `
        ...
        .goal-progress {
            background: linear-gradient(90deg, #00b894 0%, #43e97b 100%);
...
function showNotification(message, type = 'info') {
    ...
    notification.style.cssText = `
        ...
        top: 20px;
        right: 20px;
        background: ${type === 'error' ? '#ff6b9d' : type === 'success' ? '#00d4aa' : '#667eea'};
        color: white;
        padding: 15px 20px;
        border-radius: 8px;
...
function navigateTo(pageId, options = {}) {
    ...
    if (!options.skipAuthCheck && protectedToolPages.has(resolvedPageId) && !isLoggedIn()) {
        redirectToLoginForTool(resolvedPageId);
        return;
    }
    
    // Save current page to localStorage
    saveCurrentPage(resolvedPageId);
    
    // Update URL hash without triggering page reload
    if (!options.skipHashUpdate) {
        window.history.pushState(null, null, `#${resolvedPageId}`);
    }
    ...
    const target = document.getElementById(resolvedPageId);
    ...
    if (target) {
        ...
        target.style.setProperty('z-index', '1000', 'important');
        
        console.log(`Page ${resolvedPageId} displayed successfully`);
        ...
        if (resolvedPageId === 'home' && isLoggedIn()) {
            showWelcomeMessage(localStorage.getItem('bc_user_email'));
        }
        if (resolvedPageId === 'names') renderNames(currentNames || namesData);
        if (resolvedPageId === 'registry') loadRegistryState();
        if (resolvedPageId === 'family') {
            setTimeout(() => {
                initializeFamilyDashboard();
            }, 100);
        }
    ...
    document.querySelectorAll('.main-nav a').forEach(link => {
        ...
        if (link.getAttribute('data-page') === resolvedPageId) {
...
function showJoinModal() {
    alert('Welcome to Mamasafe!\n\nCreate your free account to:\n• Track your pregnancy week by week\n• Join Birth Clubs\n• Save baby names\n• Create your registry\n• Keep your care tools connected');
}
...
function handleLogin(event) {
    ...
    alert(`Welcome back, ${email}!\n\nYou are now logged in to Mamasafe.`);
    ...
    try {
        ...
        courseIds.forEach((id, index) => {
            ...
            if (isEnrolled && !badge) {
                ...
                badge.style.cssText = 'background: #00d4aa; color: white; padding:4px 8px; border-radius: 12px; font-size: 11px; position: absolute; top: 10px; right: 10px;';
        ...
        welcomeDiv.innerHTML = `
            <div class="welcome-content">
                ...
                <div>
                    ...
                    <p>You have full access to all Mamasafe features</p>
                </div>
...
function handleSignup(event) {
    ...
    // Show success message
    alert(`Welcome to Mamasafe, ${firstName}!\n\nYour account has been created successfully.\n\nYou can now:\n• Track your pregnancy\n• Join Birth Clubs\n• Save baby names\n• Create your registry`);
...
const namesData = [
    ...
    { name: 'Isaac', gender: 'boy', meaning: 'Laughter', origin: 'Hebrew' },
    { name: 'Lincoln', gender: 'boy', meaning: 'Town by the pool', origin: 'English' },
    { name: 'Hudson', gender: 'boy', meaning: 'Son of Hud', origin: 'English' },
    { name: 'Christian', gender: 'boy', meaning: 'Follower of Christ', origin: 'Latin' },
    { name: 'Ian', gender: 'boy', meaning: 'God is gracious', origin: 'Scottish' },
    { name: 'Adam', gender: 'boy', meaning: 'Man', origin: 'Hebrew' },
    ...
    { name: 'Xavier', gender: 'boy', meaning: 'Bright', origin: 'Basque' },
    { name: 'Jaxon', gender: 'boy', meaning: 'Son of Jack', origin: 'American' },
    { name: 'Theodore', gender: 'boy', meaning: 'Gift of God', origin: 'Greek' },
    { name: 'Sebastian', gender: 'boy', meaning: 'Venerable', origin: 'Greek' },
    { name: 'Jack', gender: 'boy', meaning: 'God is gracious', origin: 'English' },
    { name: 'Ryan', gender: 'boy', meaning: 'Little king', origin: 'Irish' },
    { name: 'Carson', gender: 'boy', meaning: 'Son of marsh dwellers', origin: 'Scottish' },
    { name: 'Mateo', gender: 'boy', meaning: 'Gift of God', origin: 'Spanish' },
    { name: 'Josiah', gender: 'boy', meaning: 'God supports', origin: 'Hebrew' },
    { name: 'Luke', gender: 'boy', meaning: 'Light-giving', origin: 'Latin' },
    { name: 'Isaiah', gender: 'boy', meaning: 'Salvation of the Lord', origin: 'Hebrew' },
    { name: 'Grayson', gender: 'boy', meaning: 'Son of the steward', origin: 'English' },
    { name: 'Miles', gender: 'boy', meaning: 'Soldier', origin: 'Latin' },
    { name: 'Caleb', gender: 'boy', meaning: 'Devotion to God', origin: 'Hebrew' },
    { name: 'Kayden', gender: 'boy', meaning: 'Round, hollow', origin: 'American' },
    { name: 'Beau', gender: 'boy', meaning: 'Handsome', origin: 'French' },
    { name: 'Ezra', gender: 'boy', meaning: 'Help', origin: 'Hebrew' },
    { name: 'Harrison', gender: 'boy', meaning: 'Son of Harry', origin: 'English' },
    { name: 'Jonathan', gender: 'boy', meaning: 'Gift of God', origin: 'Hebrew' },
    { name: 'Bennett', gender: 'boy', meaning: 'Blessed', origin: 'Latin' },
    { name: 'Adrian', gender: 'boy', meaning: 'Dark one', origin: 'Latin' },
    { name: 'Landon', gender: 'boy', meaning: 'Long hill', origin: 'English' },
    { name: 'Nathan', gender: 'boy', meaning: 'Given', origin: 'Hebrew' },
    { name: 'Cameron', gender: 'unisex', meaning: 'Crooked nose', origin: 'Scottish' },
    { name: 'Eli', gender: 'boy', meaning: 'Ascend', origin: 'Hebrew' },
    { name: 'Aaron', gender: 'boy', meaning: 'Mountain of strength', origin: 'Hebrew' },
    { name: 'Angel', gender: 'boy', meaning: 'Messenger', origin: 'Greek' },
    { name: 'Waylon', gender: 'boy', meaning: 'Land by the road', origin: 'English' },
    { name: 'Weston', gender: 'boy', meaning: 'West town', origin: 'English' },
    { name: 'Micah', gender: 'boy', meaning: 'Who is like God', origin: 'Hebrew' },
    { name: 'Luca', gender: 'boy', meaning: 'Bringer of light', origin: 'Italian' },
    { name: 'Emmett', gender: 'boy', meaning: 'Universal', origin: 'German' },
    { name: 'Oakley', gender: 'unisex', meaning: 'Oak meadow', origin: 'English' },
    { name: 'Diego', gender: 'boy', meaning: 'Supplanter', origin: 'Spanish' },
    { name: 'Lorenzo', gender: 'boy', meaning: 'From Laurentum', origin: 'Italian' },
    { name: 'Max', gender: 'boy', meaning: 'Greatest', origin: 'Latin' },
    { name: 'Jesus', gender: 'boy', meaning: 'God is salvation', origin: 'Hebrew' },
    { name: 'Kingston', gender: 'boy', meaning: 'King\'s town', origin: 'English' },
    { name: 'Braylon', gender: 'boy', meaning: 'Brave', origin: 'American' },
    { name: 'Kaiden', gender: 'boy', meaning: 'Round, hollow', origin: 'American' },
    { name: 'Carlos', gender: 'boy', meaning: 'Free man', origin: 'Spanish' },
    { name: 'Antonio', gender: 'boy', meaning: 'Priceless', origin: 'Latin' },
    { name: 'Dominic', gender: 'boy', meaning: 'Belonging to the Lord', origin: 'Latin' },
    { name: 'Xander', gender: 'boy', meaning: 'Defender of men', origin: 'Greek' },
    { name: 'Rafael', gender: 'boy', meaning: 'God has healed', origin: 'Hebrew' },
    { name: 'Dylan', gender: 'boy', meaning: 'Son of the sea', origin: 'Welsh' },
    { name: 'Jaxson', gender: 'boy', meaning: 'Son of Jack', origin: 'American' },
    { name: 'Brandon', gender: 'boy', meaning: 'Beacon hill', origin: 'English' },
    { name: 'Gavin', gender: 'boy', meaning: 'White hawk', origin: 'Welsh' },
    { name: 'Ryker', gender: 'boy', meaning: 'Superior', origin: 'German' },
    { name: 'Legend', gender: 'boy', meaning: 'Story', origin: 'English' },
    { name: 'Enzo', gender: 'boy', meaning: 'Home ruler', origin: 'Italian' },
    { name: 'Oscar', gender: 'boy', meaning: 'God spear', origin: 'Irish' },
    { name: 'Charlie', gender: 'boy', meaning: 'Free man', origin: 'English' },
    { name: 'Ezekiel', gender: 'boy', meaning: 'God strengthens', origin: 'Hebrew' },
    { name: 'Josue', gender: 'boy', meaning: 'God is salvation', origin: 'Hebrew' },
    { name: 'Raymond', gender: 'boy', meaning: 'Wise protector', origin: 'German' },
    { name: 'Ben', gender: 'boy', meaning: 'Son', origin: 'Hebrew' },
    { name: 'Jasper', gender: 'boy', meaning: 'Treasurer', origin: 'Persian' },
    { name: 'Emmanuel', gender: 'boy', meaning: 'God is with us', origin: 'Hebrew' },
    { name: 'Vincent', gender: 'boy', meaning: 'To conquer', origin: 'Latin' },
    { name: 'Alex', gender: 'boy', meaning: 'Defender of men', origin: 'Greek' },
    { name: 'Leonardo', gender: 'boy', meaning: 'Lion-bold', origin: 'Italian' },
    { name: 'Elliott', gender: 'boy', meaning: 'The Lord is my God', origin: 'Hebrew' },
    { name: 'Archer', gender: 'boy', meaning: 'Bowman', origin: 'English' },
    { name: 'Thaddeus', gender: 'boy', meaning: 'Courageous heart', origin: 'Aramaic' },
    { name: 'Silas', gender: 'boy', meaning: 'Forest', origin: 'Latin' },
    { name: 'Santana', gender: 'unisex', meaning: 'Saint Ana', origin: 'Spanish' },
    { name: 'Myles', gender: 'boy', meaning: 'Soldier', origin: 'Latin' },
    { name: 'Cohen', gender: 'boy', meaning: 'Priest', origin: 'Hebrew' },
    { name: 'Karter', gender: 'boy', meaning: 'Transporter of goods', origin: 'English' },
    { name: 'Trey', gender: 'boy', meaning: 'Three', origin: 'English' },
    { name: 'Kolton', gender: 'boy', meaning: 'Coal town', origin: 'English' },
    { name: 'Maxwell', gender: 'boy', meaning: 'Great stream', origin: 'Scottish' },
    { name: 'Jesse', gender: 'boy', meaning: 'Gift', origin: 'Hebrew' },
    { name: 'Bryce', gender: 'boy', meaning: 'Speckled', origin: 'Scottish' },
    { name: 'Felix', gender: 'boy', meaning: 'Lucky', origin: 'Latin' },
    { name: 'Dean', gender: 'boy', meaning: 'Valley', origin: 'English' },
    { name: 'Odin', gender: 'boy', meaning: 'Inspiration', origin: 'Norse' },
    { name: 'Kaden', gender: 'boy', meaning: 'Round, hollow', origin: 'Arabic' },
    { name: 'Gideon', gender: 'boy', meaning: 'Hewer', origin: 'Hebrew' },
    { name: 'Zion', gender: 'boy', meaning: 'Highest point', origin: 'Hebrew' },
    { name: 'Amir', gender: 'boy', meaning: 'Prince', origin: 'Arabic' },
    { name: 'Ronan', gender: 'boy', meaning: 'Little seal', origin: 'Irish' },
    { name: 'Knox', gender: 'boy', meaning: 'Round hill', origin: 'Scottish' },
    { name: 'Atlas', gender: 'boy', meaning: 'To endure', origin: 'Greek' },
    { name: 'Jude', gender: 'boy', meaning: 'Praised', origin: 'Hebrew' },
    { name: 'August', gender: 'boy', meaning: 'Great', origin: 'Latin' },
    { name: 'Tristan', gender: 'boy', meaning: 'Tumult', origin: 'Celtic' },
    { name: 'Xzavier', gender: 'boy', meaning: 'Bright', origin: 'American' },
    { name: 'Kairo', gender: 'boy', meaning: 'Victorious', origin: 'Arabic' },
    { name: 'Ace', gender: 'boy', meaning: 'Unity', origin: 'Latin' },
    { name: 'King', gender: 'boy', meaning: 'Ruler', origin: 'English' },
    { name: 'Bodhi', gender: 'boy', meaning: 'Awakening', origin: 'Sanskrit' },
    { name: 'Ephraim', gender: 'boy', meaning: 'Fruitful', origin: 'Hebrew' },
    { name: 'Dante', gender: 'boy', meaning: 'Enduring', origin: 'Italian' },
    { name: 'Maverick', gender: 'boy', meaning: 'Independent', origin: 'American' },
    { name: 'Caspian', gender: 'boy', meaning: 'Sea', origin: 'English' },
    { name: 'Orion', gender: 'boy', meaning: 'Hunter', origin: 'Greek' },
    { name: 'Stellan', gender: 'boy', meaning: 'Star', origin: 'Swedish' },
    { name: 'Zephyr', gender: 'boy', meaning: 'West wind', origin: 'Greek' },
    { name: 'Lysander', gender: 'boy', meaning: 'Liberator', origin: 'Greek' },
    { name: 'Cassius', gender: 'boy', meaning: 'Hollow', origin: 'Latin' },
    { name: 'Evander', gender: 'boy', meaning: 'Good man', origin: 'Greek' },
    { name: 'Leander', gender: 'boy', meaning: 'Lion-man', origin: 'Greek' },
    { name: 'Theron', gender: 'boy', meaning: 'Hunter', origin: 'Greek' },
    { name: 'Zane', gender: 'boy', meaning: 'God is gracious', origin: 'Hebrew' },
    { name: 'Kieran', gender: 'boy', meaning: 'Little dark one', origin: 'Irish' },
    { name: 'Callum', gender: 'boy', meaning: 'Dove', origin: 'Scottish' },
    { name: 'Finn', gender: 'boy', meaning: 'Fair', origin: 'Irish' },
    { name: 'Oisin', gender: 'boy', meaning: 'Little deer', origin: 'Irish' },
    { name: 'Cian', gender: 'boy', meaning: 'Ancient', origin: 'Irish' },
    { name: 'Tadhg', gender: 'boy', meaning: 'Poet', origin: 'Irish' },
    { name: 'Eoghan', gender: 'boy', meaning: 'Born of yew', origin: 'Irish' },
    { name: 'Saoirse', gender: 'girl', meaning: 'Freedom', origin: 'Irish' },
    { name: 'Niamh', gender: 'girl', meaning: 'Bright', origin: 'Irish' },
    { name: 'Caoimhe', gender: 'girl', meaning: 'Gentle', origin: 'Irish' },
    { name: 'Siobhan', gender: 'girl', meaning: 'God is gracious', origin: 'Irish' },
    { name: 'Aisling', gender: 'girl', meaning: 'Dream', origin: 'Irish' },
    { name: 'Aoife', gender: 'girl', meaning: 'Beauty', origin: 'Irish' },
    { name: 'Maeve', gender: 'girl', meaning: 'Intoxicating', origin: 'Irish' },
    { name: 'Clodagh', gender: 'girl', meaning: 'River name', origin: 'Irish' },
    { name: 'Orla', gender: 'girl', meaning: 'Golden princess', origin: 'Irish' },
    { name: 'Roisin', gender: 'girl', meaning: 'Little rose', origin: 'Irish' },
    { name: 'Eabha', gender: 'girl', meaning: 'Life', origin: 'Irish' },
    { name: 'Fiadh', gender: 'girl', meaning: 'Wild deer', origin: 'Irish' },
    { name: 'Isla', gender: 'girl', meaning: 'Island', origin: 'Scottish' },
    { name: 'Freya', gender: 'girl', meaning: 'Noble woman', origin: 'Norse' },
    { name: 'Astrid', gender: 'girl', meaning: 'Divine strength', origin: 'Scandinavian' },
    { name: 'Ingrid', gender: 'girl', meaning: 'Beautiful', origin: 'Scandinavian' },
    { name: 'Sigrid', gender: 'girl', meaning: 'Victory', origin: 'Scandinavian' },
    { name: 'Eira', gender: 'girl', meaning: 'Snow', origin: 'Welsh' },
    { name: 'Bronwyn', gender: 'girl', meaning: 'White breast', origin: 'Welsh' },
    { name: 'Gwenllian', gender: 'girl', meaning: 'Fair', origin: 'Welsh' },
    { name: 'Elowen', gender: 'girl', meaning: 'Elm tree', origin: 'Cornish' },
    { name: 'Morwenna', gender: 'girl', meaning: 'Waves', origin: 'Cornish' },
    { name: 'Lowenna', gender: 'girl', meaning: 'Joy', origin: 'Cornish' },
    { name: 'Kerenza', gender: 'girl', meaning: 'Love', origin: 'Cornish' },
    { name: 'Elara', gender: 'girl', meaning: 'Bright', origin: 'Greek' },
    { name: 'Lyra', gender: 'girl', meaning: 'Harp', origin: 'Greek' },
    { name: 'Cora', gender: 'girl', meaning: 'Maiden', origin: 'Greek' },
    { name: 'Nora', gender: 'girl', meaning: 'Light', origin: 'Greek' },
    { name: 'Phoebe', gender: 'girl', meaning: 'Bright', origin: 'Greek' },
    { name: 'Clio', gender: 'girl', meaning: 'Glory', origin: 'Greek' },
    { name: 'Thalia', gender: 'girl', meaning: 'To flourish', origin: 'Greek' },
    { name: 'Calliope', gender: 'girl', meaning: 'Beautiful voice', origin: 'Greek' },
    { name: 'Daphne', gender: 'girl', meaning: 'Laurel tree', origin: 'Greek' },
    { name: 'Iris', gender: 'girl', meaning: 'Rainbow', origin: 'Greek' },
    { name: 'Helen', gender: 'girl', meaning: 'Light', origin: 'Greek' },
    { name: 'Cassandra', gender: 'girl', meaning: 'Shining upon man', origin: 'Greek' },
    { name: 'Athena', gender: 'girl', meaning: 'Wisdom', origin: 'Greek' },
    { name: 'Hera', gender: 'girl', meaning: 'Queen', origin: 'Greek' },
    { name: 'Rhea', gender: 'girl', meaning: 'Flowing stream', origin: 'Greek' },
    { name: 'Selene', gender: 'girl', meaning: 'Moon', origin: 'Greek' },
    { name: 'Gaia', gender: 'girl', meaning: 'Earth', origin: 'Greek' },
    { name: 'Demeter', gender: 'girl', meaning: 'Earth mother', origin: 'Greek' },
    { name: 'Persephone', gender: 'girl', meaning: 'Bringer of destruction', origin: 'Greek' },
    ...
    { name: 'Celeste', gender: 'girl', meaning: 'Heavenly', origin: 'Latin' },
    { name: 'Serena', gender: 'girl', meaning: 'Peaceful', origin: 'Latin' },
    { name: 'Valentina', gender: 'girl', meaning: 'Strong', origin: 'Latin' },
    { name: 'Gianna', gender: 'girl', meaning: 'God is gracious', origin: 'Italian' },
    { name: 'Bianca', gender: 'girl', meaning: 'White', origin: 'Italian' },
    { name: 'Alessia', gender: 'girl', meaning: 'Defender', origin: 'Italian' },
    { name: 'Francesca', gender: 'girl', meaning: 'Free', origin: 'Italian' },
    { name: 'Chiara', gender: 'girl', meaning: 'Bright', origin: 'Italian' },
    { name: 'Gemma', gender: 'girl', meaning: 'Gem', origin: 'Italian' },
    { name: 'Martina', gender: 'girl', meaning: 'Warlike', origin: 'Italian' },
    { name: 'Viola', gender: 'girl', meaning: 'Violet', origin: 'Italian' },
    { name: 'Rosa', gender: 'girl', meaning: 'Rose', origin: 'Italian' },
    { name: 'Liliana', gender: 'girl', meaning: 'Lily', origin: 'Italian' },
    { name: 'Carmen', gender: 'girl', meaning: 'Song', origin: 'Spanish' },
    { name: 'Lucia', gender: 'girl', meaning: 'Light', origin: 'Spanish' },
    { name: 'Maria', gender: 'girl', meaning: 'Sea of bitterness', origin: 'Spanish' },
    { name: 'Alejandra', gender: 'girl', meaning: 'Defender of men', origin: 'Spanish' },
    { name: 'Valeria', gender: 'girl', meaning: 'Strength', origin: 'Spanish' },
    { name: 'Daniela', gender: 'girl', meaning: 'God is my judge', origin: 'Spanish' },
    { name: 'Paula', gender: 'girl', meaning: 'Small', origin: 'Spanish' },
    { name: 'Sara', gender: 'girl', meaning: 'Princess', origin: 'Spanish' },
    { name: 'Laura', gender: 'girl', meaning: 'Laurel', origin: 'Spanish' },
    { name: 'Clara', gender: 'girl', meaning: 'Bright', origin: 'Spanish' },
    { name: 'Adriana', gender: 'girl', meaning: 'Dark one', origin: 'Spanish' },
    { name: 'Beatriz', gender: 'girl', meaning: 'Voyager', origin: 'Spanish' },
    { name: 'Giulia', gender: 'girl', meaning: 'Youthful', origin: 'Italian' },
    { name: 'Alice', gender: 'girl', meaning: 'Noble', origin: 'Italian' },
    { name: 'Greta', gender: 'girl', meaning: 'Pearl', origin: 'Italian' },
    { name: 'Elisa', gender: 'girl', meaning: 'God is my oath', origin: 'Italian' },
    { name: 'Matilda', gender: 'girl', meaning: 'Mighty in battle', origin: 'German' },
    { name: 'Anna', gender: 'girl', meaning: 'Grace', origin: 'Hebrew' },
    { name: 'Sarah', gender: 'girl', meaning: 'Princess', origin: 'Hebrew' },
    { name: 'Rebecca', gender: 'girl', meaning: 'To bind', origin: 'Hebrew' },
    { name: 'Rachel', gender: 'girl', meaning: 'Ewe', origin: 'Hebrew' },
    { name: 'Leah', gender: 'girl', meaning: 'Weary', origin: 'Hebrew' },
    { name: 'Naomi', gender: 'girl', meaning: 'Pleasant', origin: 'Hebrew' },
    { name: 'Ruth', gender: 'girl', meaning: 'Friend', origin: 'Hebrew' },
    { name: 'Esther', gender: 'girl', meaning: 'Star', origin: 'Hebrew' },
    { name: 'Miriam', gender: 'girl', meaning: 'Sea of bitterness', origin: 'Hebrew' },
    { name: 'Judith', gender: 'girl', meaning: 'Praised', origin: 'Hebrew' },
    { name: 'Deborah', gender: 'girl', meaning: 'Bee', origin: 'Hebrew' },
    { name: 'Mary', gender: 'girl', meaning: 'Sea of bitterness', origin: 'Hebrew' },
    { name: 'Martha', gender: 'girl', meaning: 'Lady', origin: 'Aramaic' },
    { name: 'Lydia', gender: 'girl', meaning: 'From Lydia', origin: 'Greek' },
    { name: 'Priscilla', gender: 'girl', meaning: 'Ancient', origin: 'Latin' },
    { name: 'Junia', gender: 'girl', meaning: 'Youthful', origin: 'Latin' },
    { name: 'Lois', gender: 'girl', meaning: 'Better', origin: 'Greek' },
    { name: 'Eunice', gender: 'girl', meaning: 'Good victory', origin: 'Greek' },
    { name: 'Dorcas', gender: 'girl', meaning: 'Gazelle', origin: 'Greek' },
    { name: 'Tabitha', gender: 'girl', meaning: 'Gazelle', origin: 'Aramaic' },
    { name: 'Rhoda', gender: 'girl', meaning: 'Rose', origin: 'Greek' },
    { name: 'Sapphira', gender: 'girl', meaning: 'Sapphire', origin: 'Greek' },
    { name: 'Persis', gender: 'girl', meaning: 'Persian woman', origin: 'Greek' },
    { name: 'Julia', gender: 'girl', meaning: 'Youthful', origin: 'Latin' },
    { name: 'Claudia', gender: 'girl', meaning: 'Lame', origin: 'Latin' },
    { name: 'Prisca', gender: 'girl', meaning: 'Ancient', origin: 'Latin' },
    { name: 'Tryphena', gender: 'girl', meaning: 'Delicate', origin: 'Greek' },
    { name: 'Tryphosa', gender: 'girl', meaning: 'Delicate', origin: 'Greek' }
];
...
function buildFertilityDietPlan() {
    // AI-powered function - no access restrictions needed
        ...
        if (isOnlineNamesEnabled()) {
            ...
            fetchWikidataBabyNames('baby names', 50)
                .then(results => {
                    ...
                    renderNames(currentNames);
                    setNamesOnlineStatus(results.length > 0 ? 'real local names' : 'Showing offline names');
...
function confetti() {
    const colors = ['#ff6b9d', '#00d4aa', '#45b7d1', '#ff9800', '#667eea'];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('globalSearch')?.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            alert('Search functionality would search across all Mamasafe content including articles, tools, and community posts.');
        }
    });
...
function updateRealtimeData() {
    ...
    try {
        ...
        if (tempElement) {
            ...
            // Add color coding
            if (temp < 36.8) {
                tempElement.style.color = '#00b894'; // Blue for normal
            } else if (temp > 37.2) {
                tempElement.style.color = '#ff9800'; // Orange for slightly elevated
            } else {
                tempElement.style.color = '#00b894'; // Green for optimal
            }
        }
        if (tempTimeElement) {
        ...
        if (moodElement) {
            ...
            if (mood === 'Happy' || mood === 'Playful') {
                moodElement.style.color = '#00b894'; // Green
            } else if (mood === 'Sleepy' || mood === 'Calm') {
                moodElement.style.color = '#667eea'; // Purple
            } else if (mood === 'Active') {
                moodElement.style.color = '#ff9800'; // Orange
            } else if (mood === 'Hungry') {
                moodElement.style.color = '#ff4757'; // Red
            }
        }
        if (moodTimeElement) {
...
function createToddlerProfile() {
    ...
    try {
        ...
        resultDiv.style.display = 'block';
        resultDiv.innerHTML = `
            <div style="background: linear-gradient(135deg, #ff6b9d 0%, #ff9800 100%); color: white; padding: 30px; border-radius: 20px; margin-bottom: 25px; box-shadow: 0 15px 35px rgba(255, 107, 107, 0.3);">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 25px;">
                <div style="background: linear-gradient(135deg, #ff6b9d 0%, #ff8fab 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(255, 107, 107, 0.2);">
                ...
                <div style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(254, 202, 87, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">🎯</div>
                ...
                <div style="background: linear-gradient(135deg, #00d4aa 0%, #43e97b 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(0, 210, 211, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">⚡</div>
            ...
            <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 25px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.1);">
                <h4 style="margin-bottom: 20px; color: #0f2a56; font-size: 20px;">🎯 Personalized Learning Path (AI Generated)</h4>
                <div style="display: grid; gap: 15px;">
                    ${learningPath.map((activity, index) => `
                        <div style="background: white; padding: 20px; border-radius: 12px; border-left: 5px solid #ff6b9d; box-shadow: 0 3px 15px rgba(0,0,0,0.08);">
                            <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 8px;">${activity.title}</div>
            ...
            <div style="background: linear-gradient(135deg, #ffe4ec 0%, #ffe4ec 100%); padding: 20px; border-radius: 15px; box-shadow: 0 5px 20px rgba(253, 203, 110, 0.3);">
                <p style="margin: 0; color: #0f2a56; font-size: 16px; font-weight: 600;">
                    <strong>🚀 AI Recommendation:</strong> ${generateAIRecommendation(ageInMonths, focus, behaviorScore)}
                </p>
            </div>
            
            <div style="text-align: center; margin-top: 25px;">
                <button onclick="createToddlerProfile()" style="background: linear-gradient(135deg, #ff6b9d 0%, #ff9800 100%); color: white; padding: 12px 25px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; margin: 0 10px;">
                    🔄 Update Analysis
                </button>
                <button onclick="saveToddlerProfile()" style="background: linear-gradient(135deg, #00d4aa 0%, #43e97b 100%); color: white; padding: 12px 25px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; margin: 0 10px;">
                    💾 Save Profile
                </button>
            </div>
        `;
...
function saveToddlerProfile() {
    ...
    try {
        ...
        successDiv.style.cssText = `
            ...
            top: 20px;
            right: 20px;
            background: linear-gradient(135deg, #ff6b9d 0%, #ff9800 100%);
            color: white;
            padding: 15px 25px;
            border-radius: 10px;
...
function updateToddlerRealtimeData() {
    ...
    try {
        ...
        if (moodElement) {
            const mood = moods[Math.floor(Math.random() * moods.length)];
            moodElement.textContent = mood;
            moodElement.style.color = mood === 'Happy' ? '#48dbfb' : mood === 'Excited' ? '#ff6b9d' : '#ff9800';
        }
        if (moodTimeElement) {
        ...
        if (energyElement) {
            ...
            energyElement.style.color = energyLevel === 'High' || energyLevel === 'Very High' ? '#00d4aa' : '#ff9800';
        }
    ...
    try {
        ...
        resultDiv.innerHTML = `
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 25px;">
                ...
                <div style="background: linear-gradient(135deg, #ff8fab 0%, #ff6b9d 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(240, 147, 251, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">🎯</div>
                ...
                <div style="background: linear-gradient(135deg, #00d4aa 0%, #43e97b 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(79, 172, 254, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">💡</div>
            ...
            <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 25px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.1);">
                <h4 style="margin-bottom: 20px; color: #0f2a56; font-size: 20px;">🎯 Identified Triggers (AI Detected)</h4>
                <div style="display: grid; gap: 15px;">
                    ${triggers.map((trigger, index) => `
                        <div style="background: white; padding: 20px; border-radius: 12px; border-left: 5px solid #667eea; box-shadow: 0 3px 15px rgba(0,0,0,0.08);">
                            <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 8px;">${trigger.trigger}</div>
            ...
            <div style="background: linear-gradient(135deg, #fff5f7 0%, #ffe4ec 100%); padding: 25px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(255, 152, 0, 0.3);">
                <h4 style="margin-bottom: 20px; color: #0f2a56; font-size: 20px;">💡 Intervention Strategies (AI Recommended)</h4>
                <div style="display: grid; gap: 15px;">
                    ${interventions.map((intervention, index) => `
                        <div style="background: white; padding: 20px; border-radius: 12px; border-left: 5px solid #ff8fab; box-shadow: 0 3px 15px rgba(0,0,0,0.08);">
                            <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 8px;">${intervention.strategy}</div>
    ...
    resultDiv.innerHTML = `
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 40px; border-radius: 25px; margin-bottom: 35px; box-shadow: 0 25px 50px rgba(102, 126, 234, 0.4); position: relative; overflow: hidden;">
            ...
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-top: 25px; position: relative; z-index: 2;">
                ...
                <div class="ai-status" style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px);">
                    <div class="ai-status-dot" style="background: #00b894;"></div>
                ...
                <div class="ai-status" style="background: rgba(255,255,255,0.15); backdrop-filter: blur(10px);">
                    <div class="ai-status-dot" style="background: #667eea;"></div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 25px; margin-bottom: 35px;">
            <div class="metric-card-enhanced" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); position: relative; overflow: hidden;">
            ...
            <div class="metric-card-enhanced" style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%); position: relative; overflow: hidden;">
                <div style="position: absolute; top: -20px; right: -20px; width: 100px; height: 100px; background: rgba(255,255,255,0.1); border-radius: 50%;"></div>
                <div style="position: relative; z-index: 2;">
            ...
            <div class="metric-card-enhanced" style="background: linear-gradient(135deg, #667eea 0%, #ff6b9d 100%); position: relative; overflow: hidden;">
                <div style="position: absolute; top: -20px; right: -20px; width: 100px; height: 100px; background: rgba(255,255,255,0.1); border-radius: 50%;"></div>
                <div style="position: relative; z-index: 2;">
            ...
            <div class="metric-card-enhanced" style="background: linear-gradient(135deg, #00b894 0%, #00d4aa 100%); position: relative; overflow: hidden;">
                <div style="position: absolute; top: -20px; right: -20px; width: 100px; height: 100px; background: rgba(255,255,255,0.1); border-radius: 50%;"></div>
                <div style="position: relative; z-index: 2;">
        ...
        <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 20px; padding: 35px; margin-bottom: 35px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h4 style="margin: 0 0 25px 0; font-size: 24px; font-weight: 800; color: #0f2a56; display: flex; align-items: center; gap: 12px;">
                    ...
                    <div class="insight-card-premium fade-in-up" style="animation-delay: ${index * 0.15}s; border-left: 5px solid ${getInsightColor(insight.confidence)}; position: relative; overflow: hidden;">
                        ...
                        <div style="position: relative; z-index: 2;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                                <div style="flex: 1;">
                                    <div style="font-weight: 700; color: #0f2a56; font-size: 20px; margin-bottom: 12px; display: flex; align-items: center; gap: 10px;">
                                        ${insight.title}
                            ...
                            ${insight.actionable ? `
                                <div style="margin-top: 20px; padding: 20px; background: linear-gradient(135deg, #ecfdf5 0%, #ecfdf5 100%); border-radius: 15px; border-left: 4px solid #00b894; position: relative;">
                                    <div style="font-weight: 700; color: #00d4aa; margin-bottom: 12px; font-size: 16px; display: flex; align-items: center; gap: 8px;">
                                        Recommended Actions
                                    </div>
                                    <div style="display: grid; gap: 10px;">
                                        ${insight.actions.map(action => `
                                            <div style="display: flex; align-items: flex-start; gap: 10px; color: #555; font-size: 15px; line-height: 1.6;">
                                                <span style="color: #00b894; font-weight: bold; margin-top: 2px;"></span>
                                                <span>${action}</span>
                                            </div>
                                        `).join('')}
                                    </div>
        ...
        <div style="display: flex; justify-content: center; gap: 20px; margin-top: 40px; flex-wrap: wrap;">
            <button onclick="saveBehaviorAnalysis()" class="action-button-premium" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%);">
                <span class="button-icon"></span>
                <span class="button-text">Save Analysis</span>
            </button>
            <button onclick="shareBehaviorResults()" class="action-button-premium" style="background: linear-gradient(135deg, #00b894 0%, #00d4aa 100%);">
                <span class="button-icon"></span>
                <span class="button-text">Share Results</span>
            </button>
            <button onclick="printBehaviorReport()" class="action-button-premium" style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%);">
                <span class="button-icon"></span>
...
function loadAppointmentsFromStorage() {
    const stored = localStorage.getItem('mamasafe_appointments');
    if (stored) {
        try {
...
function saveAppointmentsToStorage() {
    try {
        localStorage.setItem('mamasafe_appointments', JSON.stringify(appointmentsData));
...
function loadLiveAppointments() {
        ...
        upcomingList.innerHTML = upcoming.map(apt => `
            <div class="appointment-card" data-id="${apt.id}" style="background: white; border: 2px solid #e0e0e0; border-radius: 15px; padding: 25px; display: flex; justify-content: space-between; align-items: center; transition: all 0.3s ease; border-left: 5px solid ${getAppointmentColor(apt.type)}; margin-bottom: 15px;">
                <div style="display: flex; align-items: center; gap: 20px;">
                    ...
                    <div>
                        <div style="font-weight: 700; color: #0f2a56; font-size: 18px; margin-bottom: 5px;">${getAppointmentTypeName(apt.type)}</div>
                ...
                <div style="display: flex; gap: 10px;">
                    ...
                    <button onclick="completeAppointment(${apt.id})" style="background: #00b894; color: white; border: none; padding: 10px 15px; border-radius: 8px; font-size: 13px; cursor: pointer;">✅ Complete</button>
                    <button onclick="cancelAppointment(${apt.id})" style="background: #ff8fab; color: white; border: none; padding: 10px 15px; border-radius: 8px; font-size: 13px; cursor: pointer;">❌ Cancel</button>
                </div>
            </div>
        `).join('');
    } else {
    ...
    if (past.length > 0) {
        pastList.innerHTML = past.map(apt => `
            <div class="appointment-card" data-id="${apt.id}" style="background: ${apt.status === 'cancelled' ? '#fff5f7' : '#f8f9fa'}; border: 2px solid #e0e0e0; border-radius: 15px; padding: 25px; display: flex; justify-content: space-between; align-items: center; opacity: 0.8; margin-bottom: 15px; border-left: 5px solid ${apt.status === 'cancelled' ? '#ff4757' : '#9e9e9e'};">
                <div style="display: flex; align-items: center; gap: 20px;">
                    ...
                    <div>
                        ...
                        <div style="color: #666; font-size: 16px; margin-bottom: 3px;">${formatDate(apt.date)} at ${apt.time}</div>
                        <div style="color: #666; font-size: 14px; margin-bottom: 3px;">Dr. ${apt.doctor}</div>
                        <div style="color: ${apt.status === 'cancelled' ? '#ff4757' : '#00b894'}; font-size: 14px; font-weight: 600;">${apt.status === 'cancelled' ? '❌ Cancelled' : '✅ Completed'}</div>
                    </div>
                </div>
            </div>
        `).join('');
...
function loadVaccinesFromStorage() {
    const stored = localStorage.getItem('mamasafe_vaccines');
    if (stored) {
        try {
            const parsed = JSON.parse(stored);
...
function saveVaccinesToStorage() {
    try {
        localStorage.setItem('mamasafe_vaccines', JSON.stringify(vaccinesData));
...
function handleVaccineSubmission(e) {
    ...
    const newVaccine = {
        ...
        color: status === 'completed' ? '#00b894' : status === 'scheduled' ? '#ff9800' : '#9e9e9e'
    };
    ...
    if (vaccinesData.length === 0) {
        vaccineSchedule.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 60px 40px; background: linear-gradient(135deg, #f5f7fa 0%, #e4e8ec 100%); border-radius: 20px;">
                ...
                <h3 style="color: #0f2a56; font-size: 24px; font-weight: 700; margin-bottom: 15px;">No Vaccines Recorded Yet</h3>
                ...
                <div style="display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                    ...
                    <span style="background: #00b89420; color: #00b894; padding: 8px 16px; border-radius: 20px; font-size: 14px;">✅ Mark Complete</span>
        ...
        vaccineSchedule.innerHTML = sortedVaccines.map(vaccine => `
            <div class="vaccine-card" data-id="${vaccine.id}" style="background: linear-gradient(135deg, ${vaccine.status === 'completed' ? '#ecfdf5' : vaccine.status === 'scheduled' ? '#fff5f7' : '#f5f5f5'} 0%, ${vaccine.status === 'completed' ? '#ecfdf5' : vaccine.status === 'scheduled' ? '#fff5f7' : '#fafafa'} 100%); border-radius: 20px; padding: 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); transition: all 0.3s ease; position: relative; overflow: hidden;" onmouseenter="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 15px 40px rgba(0,0,0,0.15)';" onmouseleave="this.style.transform='translateY(0)'; this.style.boxShadow='0 10px 30px rgba(0,0,0,0.1)';">
                ${vaccine.status === 'completed' ? '<div style="position: absolute; top: 15px; right: 15px; background: #00b894; color: white; border-radius: 50%; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; font-size: 16px;">✓</div>' : ''}
                <div style="display: flex; align-items: flex-start; gap: 20px;">
                    ...
                    <div style="flex: 1;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                            <div style="font-weight: 800; color: #0f2a56; font-size: 20px;">${vaccine.name}</div>
                            <span style="background: ${vaccine.status === 'completed' ? '#00b894' : vaccine.status === 'scheduled' ? '#ff9800' : '#9e9e9e'}; color: white; padding: 4px 12px; border-radius: 15px; font-size: 11px; font-weight: 700; text-transform: uppercase;">
                                ${vaccine.status}
                            </span>
                        </div>
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 15px;">
                            ...
                            <span style="background: #00b89420; color: #00b894; padding: 4px 10px; border-radius: 10px; font-size: 12px; font-weight: 600;">${formatDate(vaccine.date)}</span>
                        </div>
                ...
                <div style="margin-top: 20px; display: flex; gap: 10px;">
                    ${vaccine.status !== 'completed' ? `
                        <button onclick="markVaccineComplete(${vaccine.id})" style="flex: 1; background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; border: none; padding: 12px 20px; border-radius: 10px; font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.3s ease;">✅ Mark Complete</button>
                    ` : `
                    ...
                    <button onclick="deleteVaccine(${vaccine.id})" style="background: #fff5f7; color: #ff4757; border: 2px solid #ff4757; padding: 12px 20px; border-radius: 10px; font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.3s ease;">🗑️ Delete</button>
                </div>
            </div>
        `).join('');
    }
...
function renderVaccinationTimeline() {
    ...
    timeline.innerHTML = `
        <div style="position: relative; padding-left: 40px;">
            <!-- Timeline line -->
            <div style="position: absolute; left: 15px; top: 0; bottom: 0; width: 4px; background: linear-gradient(180deg, #00b894 0%, #667eea 50%, #ff9800 100%); border-radius: 2px;"></div>
            ...
            ${sortedVaccines.map((vaccine, index) => {
                ...
                return `
                    <div style="position: relative; margin-bottom: 30px; padding: 25px; background: ${isCompleted ? '#ecfdf5' : isScheduled ? '#fff5f7' : '#f5f5f5'}; border-radius: 15px; border-left: 5px solid ${isCompleted ? '#00b894' : isScheduled ? '#ff9800' : '#9e9e9e'};">
                        <!-- Timeline dot -->
                        <div style="position: absolute; left: -33px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; border-radius: 50%; background: ${isCompleted ? '#00b894' : isScheduled ? '#ff9800' : '#9e9e9e'}; border: 4px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.2); z-index: 1;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <div>
                                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                    ...
                                    <span style="font-weight: 800; color: #0f2a56; font-size: 18px;">${vaccine.name}</span>
                                    ${isToday ? '<span style="background: #ff6b9d; color: white; padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 700;">TODAY</span>' : ''}
                                </div>
                                <div style="color: #666; font-size: 14px; margin-bottom: 5px;">${vaccine.description}</div>
                                <div style="color: ${isCompleted ? '#00b894' : isScheduled ? '#ff9800' : '#9e9e9e'}; font-size: 14px; font-weight: 600;">${formattedDate}</div>
                            </div>
                            <span style="background: ${isCompleted ? '#00b894' : isScheduled ? '#ff9800' : '#9e9e9e'}; color: white; padding: 6px 12px; border-radius: 15px; font-size: 12px; font-weight: 700; text-transform: uppercase;">
                                ${vaccine.status}
                            </span>
                        </div>
                        ${isScheduled ? `
                            <div style="margin-top: 15px; display: flex; gap: 10px;">
                                <button onclick="markVaccineComplete(${vaccine.id})" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; border: none; padding: 8px 15px; border-radius: 8px; font-weight: 600; font-size: 12px; cursor: pointer;">✅ Complete</button>
                                <button onclick="showVaccineDetails(${vaccine.id})" style="background: #00b894; color: white; border: none; padding: 8px 15px; border-radius: 8px; font-weight: 600; font-size: 12px; cursor: pointer;">📋 Details</button>
                            </div>
                        ` : ''}
                    </div>
                `;
        ...
        <div style="background: white; border-radius: 25px; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto; position: relative; box-shadow: 0 30px 80px rgba(0,0,0,0.3); animation: slideUp 0.4s ease;">
            ...
            <div style="padding: 30px;">
                <div style="margin-bottom: 25px;">
                    <h3 style="color: #0f2a56; font-size: 18px; font-weight: 700; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                ...
                <div style="margin-bottom: 25px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div style="background: #f8f9fa; padding: 15px; border-radius: 12px;">
                            ...
                            <div style="color: #0f2a56; font-size: 16px; font-weight: 600;">${formatDate(vaccine.date)}</div>
                        </div>
                        <div style="background: #f8f9fa; padding: 15px; border-radius: 12px;">
                            <div style="color: #888; font-size: 12px; margin-bottom: 5px;">Status</div>
                            <div style="color: ${vaccine.status === 'completed' ? '#00b894' : vaccine.status === 'scheduled' ? '#ff9800' : '#9e9e9e'}; font-size: 16px; font-weight: 600; text-transform: uppercase;">${vaccine.status}</div>
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 25px;">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                        ${vaccine.sideEffects.map(effect => `
                            <span style="background: #fff5f7; color: #ff9800; padding: 6px 12px; border-radius: 20px; font-size: 13px; font-weight: 500;">${effect}</span>
                        `).join('')}
                    </div>
                </div>
                
                <div style="background: linear-gradient(135deg, #e3f2fd 0%, #fff5f7 100%); padding: 20px; border-radius: 15px; margin-bottom: 25px;">
                    <h3 style="color: #0f2a56; font-size: 16px; font-weight: 700; margin-bottom: 8px; display: flex; align-items: center; gap: 8px;">
...
function loadGrowthFromStorage() {
    const stored = localStorage.getItem('mamasafe_growth');
    if (stored) {
        try {
            const parsed = JSON.parse(stored);
...
function saveGrowthToStorage() {
    try {
        localStorage.setItem('mamasafe_growth', JSON.stringify(growthData));
    ...
    if (growthData.length === 0) {
        growthRecords.innerHTML = `
            <div style="text-align: center; padding: 60px 40px; background: linear-gradient(135deg, #f5f7fa 0%, #e4e8ec 100%); border-radius: 20px;">
                <div style="font-size: 64px; margin-bottom: 20px;">📊</div>
                <h3 style="color: #0f2a56; font-size: 24px; font-weight: 700; margin-bottom: 15px;">No Growth Records Yet</h3>
                ...
                <div style="display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                    ...
                    <span style="background: #00b89420; color: #00b894; padding: 8px 16px; border-radius: 20px; font-size: 14px;">📏 Measure Height</span>
        ...
        growthRecords.innerHTML = sortedData.map((record, index) => `
            <div class="growth-record" style="background: white; border: 2px solid #e0e0e0; border-radius: 15px; padding: 25px; transition: all 0.3s ease;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <div style="font-weight: 700; color: #0f2a56; font-size: 18px;">${formatDate(record.date)}</div>
                    <div style="display: flex; gap: 10px;">
                        ${index === 0 ? '<div style="padding: 8px 15px; background: #00b894; color: white; border-radius: 20px; font-size: 12px; font-weight: 600;">Latest</div>' : ''}
                    </div>
                </div>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 20px;">
                    <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;">
                        <div style="font-size: 28px; margin-bottom: 8px;">⚖️</div>
                        <div style="font-weight: 700; color: #0f2a56; font-size: 18px;">${record.weight} kg</div>
                        <div style="color: #666; font-size: 13px;">Weight</div>
                    </div>
                    <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;">
                        <div style="font-size: 28px; margin-bottom: 8px;">📏</div>
                        <div style="font-weight: 700; color: #0f2a56; font-size: 18px;">${record.height} cm</div>
                        <div style="color: #666; font-size: 13px;">Height</div>
                    </div>
                    ${record.head ? `
                    <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;">
                        <div style="font-size: 28px; margin-bottom: 8px;">🧠</div>
                        <div style="font-weight: 700; color: #0f2a56; font-size: 18px;">${record.head} cm</div>
                        <div style="color: #666; font-size: 13px;">Head Circ.</div>
                    </div>
                    ` : ''}
                    <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;">
                        <div style="font-size: 28px; margin-bottom: 8px;">📊</div>
                        <div style="font-weight: 700; color: #0f2a56; font-size: 18px;">${record.bmi}</div>
                ...
                <div style="margin-top: 15px; display: flex; gap: 10px;">
                    ...
                    <button onclick="deleteGrowthRecord(${record.id})" style="background: #fff5f7; color: #ff4757; border: 2px solid #ff4757; padding: 10px 15px; border-radius: 8px; font-size: 13px; cursor: pointer; font-weight: 600;">🗑️ Delete</button>
                </div>
            </div>
        `).join('');
    }
        ...
        <div style="background: white; border-radius: 25px; max-width: 1000px; width: 90%; max-height: 90vh; overflow-y: auto; position: relative; box-shadow: 0 30px 80px rgba(0,0,0,0.3);">
            ...
            <div style="padding: 30px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 30px;">
                    ...
                    <div style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; padding: 20px; border-radius: 15px; text-align: center;">
                        <div style="font-size: 36px; margin-bottom: 10px;">😴</div>
                    ...
                    <div style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%); color: white; padding: 20px; border-radius: 15px; text-align: center;">
                        <div style="font-size: 36px; margin-bottom: 10px;">⏰</div>
                    ...
                    <div style="background: linear-gradient(135deg, #00b894 0%, #00d4aa 100%); color: white; padding: 20px; border-radius: 15px; text-align: center;">
                ...
                <div style="background: #f8f9fa; padding: 25px; border-radius: 15px; margin-bottom: 30px;">
                    <h3 style="margin: 0 0 20px 0; color: #0f2a56; font-size: 20px; font-weight: 700;">Record Sleep Session</h3>
                ...
                <div style="background: white; border: 2px solid #e0e0e0; border-radius: 15px; padding: 20px; margin-bottom: 30px;">
                    <h3 style="margin: 0 0 20px 0; color: #0f2a56; font-size: 18px; font-weight: 700;">Weekly Sleep Pattern</h3>
                    <canvas id="sleepChart" width="900" height="200"></canvas>
                </div>
                
                <!-- Recent Sleep Entries -->
                <div style="margin-bottom: 30px;">
                    <h3 style="margin: 0 0 20px 0; color: #0f2a56; font-size: 20px; font-weight: 700;">Recent Sleep Entries</h3>
                ...
                <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 15px; padding: 25px;">
                    <h3 style="margin: 0 0 20px 0; color: #0f2a56; font-size: 20px; font-weight: 700;">AI Sleep Recommendations</h3>
                    <div style="display: grid; gap: 15px;">
                        <div style="background: white; border-left: 4px solid #00b894; border-radius: 10px; padding: 20px;">
                            <div style="font-weight: 700; color: #00d4aa; margin-bottom: 10px; font-size: 16px;">✓ Excellent Sleep Pattern</div>
                        ...
                        <div style="background: white; border-left: 4px solid #00b894; border-radius: 10px; padding: 20px;">
                            <div style="font-weight: 700; color: #512da8; margin-bottom: 10px; font-size: 16px;">💡 Optimize Nap Schedule</div>
                        ...
                        <div style="background: white; border-left: 4px solid #ff9800; border-radius: 10px; padding: 20px;">
                            <div style="font-weight: 700; color: #ff9800; margin-bottom: 10px; font-size: 16px;">🌙 Bedtime Routine Tips</div>
    ...
    entriesList.innerHTML = sleepEntries.map(entry => `
        <div style="background: white; border: 2px solid #e0e0e0; border-radius: 15px; padding: 20px; display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <div style="font-weight: 700; color: #0f2a56; font-size: 16px;">${entry.type === 'night' ? 'Night Sleep' : 'Nap'}</div>
                    <div style="color: #666; font-size: 14px;">${entry.date}</div>
                    <div style="color: #666; font-size: 14px;">${entry.start} - ${entry.end}</div>
                </div>
            </div>
            <div style="text-align: right;">
                <div style="font-weight: 700; color: #0f2a56; font-size: 18px;">${entry.duration}</div>
...
function drawSleepChart() {
    ...
    data.forEach((hours, index) => {
        ...
        gradient.addColorStop(1, '#0f2a56');
    ...
    modal.innerHTML = `
        <div style="background: white; border-radius: 20px; max-width: 500px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px rgba(0,0,0,0.3); animation: slideUp 0.3s ease;">
            ...
            <div style="padding: 25px;">
                <!-- Duration Card -->
                <div style="background: linear-gradient(135deg, #ecfdf5 0%, #a8edea 100%); padding: 25px; border-radius: 15px; text-align: center; margin-bottom: 20px; border-left: 4px solid #00b894;">
                    <div style="font-size: 48px; font-weight: 800; color: #00b894; line-height: 1;">${durationHours}<span style="font-size: 24px;">h</span> ${durationMinutes > 0 ? durationMinutes + '<span style="font-size: 24px;">m</span>' : ''}</div>
                    <div style="font-size: 14px; color: #666; margin-top: 8px;">Total Sleep Duration</div>
                    <div style="font-size: 16px; font-weight: 600; color: #00b894; margin-top: 10px; padding: 6px 16px; background: rgba(76,175,80,0.15); border-radius: 20px; display: inline-block;">${assessment.rating}</div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 12px; text-align: center;">
                        ...
                        <div style="font-size: 18px; font-weight: 700; color: #0f2a56;">${formatTime(start)}</div>
                        <div style="font-size: 12px; color: #666;">Sleep Start</div>
                    </div>
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 12px; text-align: center;">
                        <div style="font-size: 20px; margin-bottom: 5px;">☀️</div>
                        <div style="font-size: 18px; font-weight: 700; color: #0f2a56;">${formatTime(end)}</div>
                ...
                <div style="padding: 20px; background: ${assessment.bgColor}; border-radius: 15px; margin-bottom: 20px; border-left: 4px solid ${assessment.borderColor};">
                    ...
                    <p style="margin: 0; color: #0f2a56; font-size: 15px; line-height: 1.5;">${assessment.message}</p>
                </div>
            ...
            <div style="padding: 0 25px 25px 25px; text-align: center; display: flex; gap: 10px;">
                ...
                <button onclick="saveAndCloseSleepModal('${date}', '${start}', '${end}', '${type}', ${totalHours.toFixed(2)})" style="flex: 1; background: #00b894; color: white; border: none; padding: 15px 30px; border-radius: 10px; font-weight: 600; font-size: 16px; cursor: pointer;">💾 Save Entry</button>
            </div>
        </div>
    `;
...
function saveAndCloseSleepModal(date, start, end, type, duration) {
    // Save to localStorage
    const sleepEntries = JSON.parse(localStorage.getItem('mamasafe_sleep_entries') || '[]');
    sleepEntries.push({
        id: Date.now(),
        date,
        start,
    ...
    localStorage.setItem('mamasafe_sleep_entries', JSON.stringify(sleepEntries));
...
function getInsightColor(confidence) {
    if (confidence >= 90) return '#00b894';
    if (confidence >= 80) return '#ff9800';
    if (confidence >= 70) return '#ff527d';
    return '#667eea';
}
...
function celebrateSkillsComplete() {
    ...
    celebration.style.cssText = `
        ...
        background: linear-gradient(135deg, #00b894 0%, #43e97b 100%);
        color: white;
        padding: 30px 40px;
        border-radius: 20px;
        font-size: 20px;
...
function displayLearningOptimizationResults(learningPath, name, goal, skills, age) {
    ...
    resultDiv.innerHTML = `
        ...
        <div style="background: linear-gradient(135deg, #ff8fab 0%, #ff6b9d 100%); color: white; padding: 35px; border-radius: 20px; margin-bottom: 30px; box-shadow: 0 20px 40px rgba(240, 147, 251, 0.3); position: relative; overflow: hidden;">
            <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(45deg, transparent, rgba(255,255,255,0.1), transparent); animation: dataFlow 4s linear infinite;"></div>
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
            <div class="metric-card" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%);">
            ...
            <div class="metric-card" style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%);">
                <div style="font-size: 36px; margin-bottom: 15px;">🎯</div>
            ...
            <div class="metric-card" style="background: linear-gradient(135deg, #667eea 0%, #ff6b9d 100%);">
                <div style="font-size: 36px; margin-bottom: 15px;">📊</div>
            ...
            <div class="metric-card" style="background: linear-gradient(135deg, #00b894 0%, #00d4aa 100%);">
                <div style="font-size: 36px; margin-bottom: 15px;">🏆</div>
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 30px;">
            ${learningPath.activities.map((activity, index) => `
                <div class="insight-card fade-in-up" style="animation-delay: ${index * 0.1}s; border-left-color: #ff8fab; cursor: pointer;" onclick="startLearningActivity('${activity.title}')">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                        <div style="flex: 1;">
                            <div style="font-weight: 700; color: #0f2a56; font-size: 18px; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                                ${activity.title}
                                <span class="confidence-badge" style="background: #ff8fab;">
                                    ${activity.duration}
                                </span>
                            </div>
                    ...
                    <div style="margin-top: 15px; padding: 15px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px; border-left: 4px solid #00b894;">
                        <div style="font-weight: 600; color: #0f2a56; margin-bottom: 8px; font-size: 14px;">🎮 Interactive Features:</div>
        ...
        <div style="background: linear-gradient(135deg, #ffe4ec 0%, #ffe4ec 100%); padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(253, 203, 110, 0.3); margin-bottom: 30px; position: relative;">
            <div style="position: absolute; top: -10px; right: -10px; width: 30px; height: 30px; background: #ff6b9d; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; box-shadow: 0 5px 15px rgba(255, 107, 107, 0.4);">
                🎮
            </div>
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <div style="font-weight: 700; color: #0f2a56; font-size: 18px; margin-bottom: 5px;">Gamified Learning Journey</div>
            ...
            <div style="margin-top: 20px; padding: 15px; background: rgba(255,255,255,0.3); border-radius: 10px;">
                <div style="font-size: 12px; color: #0f2a56; font-weight: 600; margin-bottom: 8px;">🎯 Learning Rewards:</div>
        ...
        <div style="text-align: center; margin-top: 40px; display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
            <button onclick="startLearningJourney()" class="ai-button-secondary" style="background: linear-gradient(135deg, #ff8fab 0%, #ff6b9d 100%); color: white;">
                🚀 Start Learning Journey
            </button>
            <button onclick="saveLearningPath()" class="ai-button-secondary" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white;">
                💾 Save Learning Path
            </button>
            <button onclick="shareLearningResults()" class="ai-button-secondary" style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%); color: white;">
                📤 Share Progress
            </button>
            <button onclick="printLearningReport()" class="ai-button-secondary" style="background: linear-gradient(135deg, #00b894 0%, #00d4aa 100%); color: white;">
                🖨️ Print Report
            </button>
        </div>
    `;
...
function analyzeEmotionRecognition() {
    ...
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #00d4aa 0%, #43e97b 100%); color: white; padding: 30px; border-radius: 20px; margin-bottom: 25px; box-shadow: 0 15px 35px rgba(79, 172, 254, 0.3);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 25px;">
            <div style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; padding: 25px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(76, 175, 80, 0.2);">
                <div style="font-size: 32px; margin-bottom: 10px;">📊</div>
            ...
            <div style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%); color: white; padding: 25px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(255, 152, 0, 0.2);">
                <div style="font-size: 32px; margin-bottom: 10px;">🧠</div>
            ...
            <div style="background: linear-gradient(135deg, #667eea 0%, #ff6b9d 100%); color: white; padding: 25px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(102, 126, 234, 0.2);">
                <div style="font-size: 32px; margin-bottom: 10px;">🎯</div>
        ...
        <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 25px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.1);">
            <h4 style="margin-bottom: 20px; color: #0f2a56; font-size: 20px; font-weight: 700;">💞 Advanced Emotional Insights</h4>
            <div style="display: grid; gap: 15px;">
                ${emotionAnalysis.insights.map((insight, index) => `
                    <div style="background: white; padding: 20px; border-radius: 12px; border-left: 5px solid #00d4aa; box-shadow: 0 3px 15px rgba(0,0,0,0.08);">
                        <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 8px;">${insight.title}</div>
                        <div style="color: #666; font-size: 14px; line-height: 1.5;">${insight.description}</div>
                        <div style="margin-top: 10px; padding: 8px 12px; background: #00d4aa; color: white; border-radius: 8px; display: inline-block; font-size: 12px; font-weight: 600;">
        ...
        <div style="text-align: center; margin-top: 30px;">
            <button onclick="openEmotionAI()" style="background: linear-gradient(135deg, #00d4aa 0%, #43e97b 100%); color: white; padding: 15px 30px; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; margin: 0 10px;">
                🔄 New Analysis
            </button>
            <button onclick="saveEmotionAnalysis()" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; padding: 15px 30px; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; margin: 0 10px;">
                💾 Save Report
            </button>
        </div>
    `;
...
function optimizeLearningPath() {
    ...
    try {
        ...
        resultDiv.style.display = 'block';
        resultDiv.innerHTML = `
            <div style="background: linear-gradient(135deg, #ff8fab 0%, #ff6b9d 100%); color: white; padding: 30px; border-radius: 20px; margin-bottom: 25px; box-shadow: 0 15px 35px rgba(240, 147, 251, 0.3);">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 25px;">
                ...
                <div style="background: linear-gradient(135deg, #ff9800 0%, #ffd700 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(250, 112, 154, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">📈</div>
            ...
            <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 25px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.1);">
                <h4 style="margin-bottom: 20px; color: #0f2a56; font-size: 20px;">🎯 Identified Skill Gaps (AI Analysis)</h4>
                <div style="display: grid; gap: 15px;">
                    ${skillGaps.map((gap, index) => `
                        <div style="background: white; padding: 20px; border-radius: 12px; border-left: 5px solid #ff8fab; box-shadow: 0 3px 15px rgba(0,0,0,0.08);">
                            <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 8px;">${gap.skill}</div>
            ...
            <div style="text-align: center; margin-top: 25px;">
                <button onclick="optimizeLearningPath()" style="background: linear-gradient(135deg, #ff8fab 0%, #ff6b9d 100%); color: white; padding: 12px 25px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; margin: 0 10px;">
                    🔄 Re-optimize
                </button>
...
function createSmartBabyProfile() {
    ...
    try {
        ...
        resultDiv.innerHTML = `
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 25px;">
                <div style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(76, 175, 80, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">📊</div>
                ...
                <div style="background: linear-gradient(135deg, #00b894 0%, #03a9f4 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(33, 150, 243, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">🧠</div>
                ...
                <div style="background: linear-gradient(135deg, #ff9800 0%, #ff8fab 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(255, 152, 0, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">🥗</div>
                ...
                <div style="background: linear-gradient(135deg, #667eea 0%, #ff6b9d 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(102, 126, 234, 0.2);">
                    <div style="font-size: 32px; margin-bottom: 10px;">😴</div>
            ...
            <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 25px; border-radius: 15px; margin-bottom: 25px; box-shadow: 0 5px 20px rgba(0,0,0,0.1);">
                <h4 style="margin-bottom: 20px; color: #0f2a56; font-size: 20px;">🎯 Next Developmental Milestones (AI Predicted)</h4>
                <div style="display: grid; gap: 15px;">
                    ${nextMilestones.map((milestone, index) => `
                        <div style="background: white; padding: 20px; border-radius: 12px; border-left: 5px solid #00b894; box-shadow: 0 3px 15px rgba(0,0,0,0.08);">
                            <div style="font-weight: 700; color: #0f2a56; font-size: 16px; margin-bottom: 8px;">${milestone.title}</div>
            ...
            <div style="text-align: center; margin-top: 25px;">
                ...
                <button onclick="saveBabyProfile()" style="background: linear-gradient(135deg, #00b894 0%, #43e97b 100%); color: white; padding: 12px 25px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; margin: 0 10px;">
                    💾 Save Profile
                </button>
            </div>
        `;
...
function runGrowthPredictionAI() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #00b894 0%, #45a049 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
        ...
        <div style="background: #ecfdf5; padding: 20px; border-radius: 10px;">
            <h4 style="margin-bottom: 15px; color: #00b894;">📈 Growth Recommendations</h4>
...
function generateFeedingSchedule() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #ff6b9d 0%, #ff6b9d 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    scheduleDisplay.innerHTML = schedule.feedingTimes.map((time, index) => `
        <div style="background: white; padding: 20px; border-radius: 15px; box-shadow: 0 5px 20px rgba(0,0,0,0.1); display: flex; align-items: center; gap: 20px;">
            <div style="background: linear-gradient(135deg, #ff6b9d 0%, #ff6b9d 100%); color: white; width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0;">
                ${index + 1}
            </div>
            <div style="flex: 1;">
                <div style="font-size: 18px; font-weight: bold; color: #0f2a56; margin-bottom: 5px;">${time.time}</div>
            ...
            <div style="display: grid; gap: 15px;">
                ${analysis.recommendations.map(rec => `
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 10px; border-left: 4px solid #667eea;">
                        <div style="font-weight: 600; color: #0f2a56; margin-bottom: 5px;">${rec.title}</div>
            ...
            <div style="display: grid; gap: 10px;">
                ${analysis.schedule.map(item => `
                    <div style="background: #e3f2fd; padding: 15px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div style="font-weight: 600; color: #00b894;">${item.time}</div>
...
function runDevelopmentAnalysis() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #ff9800 0%, #ffd700 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    reportDiv.innerHTML = `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); margin-bottom: 20px;">
            <h3 style="color: #ff9800; margin-bottom: 20px;">📈 Development Progress by Area</h3>
            <div style="display: grid; gap: 15px;">
                ${analysis.areaBreakdown.map(area => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            <div style="font-weight: 600; color: #0f2a56;">${area.name}</div>
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #ff9800; margin-bottom: 20px;">🎯 Personalized Activities</h3>
            <div style="display: grid; gap: 15px;">
                ${analysis.recommendedActivities.map(activity => `
                    <div style="background: #fff5f7; padding: 15px; border-radius: 10px; border-left: 4px solid #ff9800;">
...
function runHealthAssessment() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #ff8fab 0%, #ff6b9d 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    reportDiv.innerHTML = `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); margin-bottom: 20px;">
            <h3 style="color: #ff8fab; margin-bottom: 20px;">📋 Health Analysis Report</h3>
            <div style="display: grid; gap: 15px;">
                ${assessment.analysis.map(item => `
                    <div style="background: ${item.severity === 'high' ? '#fff5f7' : item.severity === 'medium' ? '#fff5f7' : '#ecfdf5'}; padding: 15px; border-radius: 10px; border-left: 4px solid ${item.severity === 'high' ? '#ff4757' : item.severity === 'medium' ? '#ff9800' : '#00b894'};">
                        <div style="font-weight: 600; color: ${item.severity === 'high' ? '#ff4757' : item.severity === 'medium' ? '#ff9800' : '#00b894'}; margin-bottom: 5px;">${item.title}</div>
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #ff8fab; margin-bottom: 20px;">⚡ Recommendations</h3>
            <div style="display: grid; gap: 15px;">
                ${assessment.recommendations.map(rec => `
                    <div style="background: #fff5f7; padding: 15px; border-radius: 10px; display: flex; align-items: center; gap: 15px;">
                        <div style="background: #667eea; color: white; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0;">
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); margin-bottom: 20px;">
            ...
            <div style="display: grid; gap: 15px;">
                ${calculation.breakdown.map(item => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            <div style="font-weight: 600; color: #0f2a56;">${item.category}</div>
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            ...
            <div style="display: grid; gap: 15px;">
                ${calculation.savingsTips.map(tip => `
                    <div style="background: #ecfdf5; padding: 15px; border-radius: 10px; border-left: 4px solid #00b894;">
                        <div style="font-weight: 600; color: #00b894; margin-bottom: 5px;">${tip.title}</div>
                        <div style="color: #666; margin-bottom: 10px;">${tip.description}</div>
                        <div style="background: #00b894; color: white; padding: 5px 15px; border-radius: 15px; font-size: 12px; display: inline-block;">
...
function analyzeDevelopment(age, areas) {
    ...
    return {
        ...
        totalMilestones: 22,
        areaBreakdown: [
            { name: 'Motor Skills', score: Math.round(overallScore + Math.random() * 10 - 5), color: '#00b894', status: 'Developing well' },
            { name: 'Cognitive', score: Math.round(overallScore + Math.random() * 10 - 5), color: '#00b894', status: 'Age appropriate' },
            { name: 'Social', score: Math.round(overallScore + Math.random() * 10 - 5), color: '#ff9800', status: 'Progressing nicely' },
            { name: 'Language', score: Math.round(overallScore + Math.random() * 10 - 5), color: '#667eea', status: 'On track' }
        ],
...
function generateNutritionPlan() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #00b894 0%, #00d4aa 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
            ...
            <div style="display: grid; gap: 15px;">
                ${rec.items.map(item => `
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div style="font-weight: 600; color: #0f2a56;">${item.name}</div>
...
function startActivityTracking() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #fd79a8 0%, #ffe4ec 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            ...
            <div style="display: grid; gap: 15px;">
                ${activityData.analytics.map(metric => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            <div style="font-weight: 600; color: #0f2a56;">${metric.name}</div>
...
function runSafetyAnalysis() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #ff6b9d 0%, #ff8fab 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    dashboardDiv.innerHTML = `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #ff6b9d; margin-bottom: 20px;">🚨 Safety Monitoring Dashboard</h3>
            <div style="display: grid; gap: 15px;">
                ${safetyReport.recommendations.map(rec => `
                    <div style="background: ${rec.severity === 'high' ? '#fff5f7' : rec.severity === 'medium' ? '#fff5f7' : '#ecfdf5'}; padding: 15px; border-radius: 10px; border-left: 4px solid ${rec.severity === 'high' ? '#ff4757' : rec.severity === 'medium' ? '#ff9800' : '#00b894'};">
                        <div style="font-weight: 600; color: ${rec.severity === 'high' ? '#ff4757' : rec.severity === 'medium' ? '#ff9800' : '#00b894'}; margin-bottom: 5px;">${rec.title}</div>
                        <div style="color: #666;">${rec.description}</div>
                    </div>
                `).join('')}
            </div>
...
function analyzeMood() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #a29bfe 0%, #43e97b 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            ...
            <div style="display: grid; gap: 15px;">
                ${moodAnalysis.insights.map(insight => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <div style="font-weight: 600; color: #0f2a56; margin-bottom: 10px;">${insight.title}</div>
...
function generateLearningPlan() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #00d4aa 0%, #43e97b 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    activitiesDiv.innerHTML = learningPlan.recommendedActivities.map(activity => `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #00d4aa; margin-bottom: 15px;">${activity.title}</h3>
            <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 15px;">
                <div style="color: #666; margin-bottom: 10px;">${activity.description}</div>
                <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px;">
                    <span style="background: #00d4aa; color: white; padding: 5px 15px; border-radius: 15px; font-size: 12px;">
                        ${activity.duration}
                    </span>
                    <span style="background: #43e97b; color: white; padding: 5px 15px; border-radius: 15px; font-size: 12px;">
                        ${activity.category}
                    </span>
                </div>
            </div>
...
function generateCareSchedule() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #43e97b 0%, #a29bfe 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    calendarDiv.innerHTML = `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #43e97b; margin-bottom: 20px;">📋 Smart Care Calendar</h3>
            <div style="display: grid; gap: 15px;">
                ${careSchedule.schedule.map(item => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div style="font-weight: 600; color: #0f2a56;">${item.time}</div>
                            <div style="color: #666; font-size: 14px;">${item.activity}</div>
                        </div>
                        <div style="background: #43e97b; color: white; padding: 5px 15px; border-radius: 15px; font-size: 12px;">
                            ${item.type}
                        </div>
                    </div>
                `).join('')}
...
function joinCommunity() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #ff8fab 0%, #fd79a8 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    hubDiv.innerHTML = `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #ff8fab; margin-bottom: 20px;">💬 Parent Community Hub</h3>
            <div style="display: grid; gap: 15px;">
                ${communityProfile.recommendedGroups.map(group => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <div style="font-weight: 600; color: #0f2a56; margin-bottom: 10px;">${group.name}</div>
                        <div style="color: #666; margin-bottom: 15px;">${group.description}</div>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="color: #999; font-size: 14px;">${group.members} members</div>
                            <button style="background: #ff8fab; color: white; padding: 8px 20px; border: none; border-radius: 15px; font-size: 12px; cursor: pointer;">
                                Join Group
                            </button>
                        </div>
                    </div>
...
function createMemoryBook() {
    ...
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = `
        <div style="background: linear-gradient(135deg, #ffe4ec 0%, #ff6b9d 100%); color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px;">
    ...
    timelineDiv.innerHTML = `
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="color: #ffe4ec; margin-bottom: 20px;">🌟 Memory Timeline</h3>
            <div style="display: grid; gap: 15px;">
                ${memoryBook.timeline.map(memory => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; border-left: 4px solid #ffe4ec;">
                        <div style="font-weight: 600; color: #0f2a56; margin-bottom: 10px;">${memory.date}</div>
        ...
        <div style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            ...
            <div style="display: grid; gap: 15px;">
                ${weatherAdvice.recommendations.map(rec => `
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <div style="font-weight: 600; color: #0f2a56; margin-bottom: 10px;">${rec.category}</div>
...
function showAccountPanel(panel) {
    ...
    const all = document.querySelectorAll('#38f9d7ount .account-panel');
    ...
    modal.innerHTML = `
        <div style="background: white; border-radius: 20px; max-width: 500px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px rgba(0,0,0,0.3); animation: slideUp 0.3s ease;">
            ...
            <div style="padding: 25px;">
                <!-- Duration Card -->
                <div style="background: linear-gradient(135deg, #ecfdf5 0%, #a8edea 100%); padding: 25px; border-radius: 15px; text-align: center; margin-bottom: 20px; border-left: 4px solid #00b894;">
                    <div style="font-size: 48px; font-weight: 800; color: #00b894; line-height: 1;">${hours}<span style="font-size: 24px;">h</span> ${minutes > 0 ? minutes + '<span style="font-size: 24px;">m</span>' : ''}</div>
                    <div style="font-size: 14px; color: #666; margin-top: 8px;">Total Sleep Duration</div>
                    <div style="font-size: 16px; font-weight: 600; color: #00b894; margin-top: 10px; padding: 6px 16px; background: rgba(76,175,80,0.15); border-radius: 20px; display: inline-block;">${assessment.rating}</div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 12px; text-align: center;">
                        <div style="font-size: 20px; margin-bottom: 5px;">🌙</div>
                        <div style="font-size: 18px; font-weight: 700; color: #0f2a56;">${formatTime12Hour(startTime)}</div>
                        <div style="font-size: 12px; color: #666;">Sleep Start</div>
                    </div>
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 12px; text-align: center;">
                        <div style="font-size: 20px; margin-bottom: 5px;">☀️</div>
                        <div style="font-size: 18px; font-weight: 700; color: #0f2a56;">${formatTime12Hour(endTime)}</div>
                        <div style="font-size: 12px; color: #666;">Wake Up</div>
                    </div>
                </div>
            ...
            <div style="padding: 0 25px 25px 25px; text-align: center; display: flex; gap: 10px;">
                ...
                <button onclick="saveAndCloseSleepTrackResult('${startTime}', '${endTime}', '${quality}', ${hours}, ${minutes})" style="flex: 1; background: #00b894; color: white; border: none; padding: 15px 30px; border-radius: 10px; font-weight: 600; font-size: 16px; cursor: pointer;">💾 Save Entry</button>
            </div>
        </div>
    `;
...
function saveAndCloseSleepTrackResult(startTime, endTime, quality, hours, minutes) {
    // Save to localStorage
    const sleepEntries = JSON.parse(localStorage.getItem('mamasafe_sleep_track_entries') || '[]');
    sleepEntries.push({
        id: Date.now(),
        startTime,
        endTime,
    ...
    localStorage.setItem('mamasafe_sleep_track_entries', JSON.stringify(sleepEntries));
        ...
        <div style="background: white; border-radius: 20px; max-width: 600px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px rgba(0,0,0,0.3); animation: slideUp 0.3s ease;">
            ...
            <div style="padding: 25px;">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-bottom: 20px;">
                    <div style="background: linear-gradient(135deg, #ecfdf5 0%, #a8edea 100%); padding: 20px; border-radius: 15px; text-align: center; border-left: 4px solid #00b894;">
                        <div style="font-size: 28px; font-weight: 700; color: #00b894;">${weight} kg</div>
                        <div style="font-size: 14px; color: #666; margin-top: 5px;">Weight</div>
                        <div style="font-size: 16px; font-weight: 600; color: #00b894; margin-top: 5px; padding: 4px 12px; background: rgba(76,175,80,0.1); border-radius: 20px; display: inline-block;">${Math.round(weightPercentile)}th percentile</div>
                    </div>
                    <div style="background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%); padding: 20px; border-radius: 15px; text-align: center; border-left: 4px solid #00b894;">
                        <div style="font-size: 28px; font-weight: 700; color: #00b894;">${height} cm</div>
                        <div style="font-size: 14px; color: #666; margin-top: 5px;">Height</div>
                        <div style="font-size: 16px; font-weight: 600; color: #00b894; margin-top: 5px; padding: 4px 12px; background: rgba(33,150,243,0.1); border-radius: 20px; display: inline-block;">${Math.round(heightPercentile)}th percentile</div>
                    </div>
                    <div style="background: linear-gradient(135deg, #fff5f7 0%, #ffe4ec 100%); padding: 20px; border-radius: 15px; text-align: center; border-left: 4px solid #ff9800;">
                    ...
                    ${headCirc ? `
                    <div style="background: linear-gradient(135deg, #fff5f7 0%, #a8edea 100%); padding: 20px; border-radius: 15px; text-align: center; border-left: 4px solid #667eea;">
                        <div style="font-size: 28px; font-weight: 700; color: #667eea;">${headCirc} cm</div>
                        <div style="font-size: 14px; color: #666; margin-top: 5px;">Head Circumference</div>
                    </div>
                    ` : ''}
                </div>
                
                <div style="padding: 20px; background: linear-gradient(135deg, #ecfdf5 0%, #a8edea 100%); border-radius: 15px; margin-bottom: 20px;">
                    <strong style="color: #00d4aa; font-size: 18px; display: block; margin-bottom: 10px;">Interpretation:</strong>
                    <p style="margin: 0; color: #0f2a56; font-size: 16px; line-height: 1.6;">${getGrowthInterpretation(weightPercentile, heightPercentile)}</p>
                </div>
...
function createBabyProfile() {
    ...
    profileResult.innerHTML = `
        <div style="background: linear-gradient(135deg, #ecfdf5 0%, #a8edea 100%); padding: 25px; border-radius: 15px; border: 1px solid #a8edea;">
            <h4 style="color: #00d4aa; margin-bottom: 20px; font-size: 20px; font-weight: 700;">👶 Baby Profile Created!</h4>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                <div>
                    <strong style="color: #0f2a56;">Name:</strong> ${babyName}
                </div>
                <div>
                    <strong style="color: #0f2a56;">Age:</strong> ${ageInWeeks} weeks (${ageInDays} days)
                </div>
                <div>
                    <strong style="color: #0f2a56;">Weight:</strong> ${babyWeight} kg
                </div>
                <div>
                    <strong style="color: #0f2a56;">Length:</strong> ${babyLength} cm
                </div>
                <div>
                    <strong style="color: #0f2a56;">Feeding:</strong> ${feedingMethod.charAt(0).toUpperCase() + feedingMethod.slice(1)}
                </div>
                <div>
                    <strong style="color: #0f2a56;">Birth Date:</strong> ${new Date(babyBirthDate).toLocaleDateString()}
                </div>
            </div>
            <div style="background: white; padding: 15px; border-radius: 10px; border: 1px solid #e0e0e0;">
                <h5 style="color: #0f2a56; margin-bottom: 10px; font-weight: 600;">📊 Development Milestones:</h5>
...
// Pregnancy Week-by-Week Tracker Data
const pregnancyWeekData = {
    1: {
        title: "Conception & Early Development",
        babySize: "Poppy seed",
        babyGrowth: "The fertilized egg implants in the uterus and begins dividing rapidly. Your baby is just a tiny cluster of cells.",
        motherChanges: "You may not feel pregnant yet, but your body is producing hCG hormone. Some women experience very mild cramping or spotting.",
        careFocus: "Start taking prenatal vitamins with folic acid. Avoid alcohol, smoking, and certain medications.",
        askProvider: "Discuss any medications you're taking and confirm your due date calculation.",
        checklist: ["Start prenatal vitamins", "Schedule first prenatal appointment", "Begin tracking your cycle", "Avoid harmful substances"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "Fever over 101°F"],
        milestone: "Conception occurs and baby's genetic blueprint is established"
    },
    2: {
        title: "Cell Division & Implantation",
        babySize: "Sesame seed",
        babyGrowth: "The embryo is now a blastocyst with three layers that will form all organs and tissues. Amniotic sac is forming.",
        motherChanges: "You might miss your period and get a positive pregnancy test. Some women feel tired or experience mild nausea.",
        careFocus: "Continue prenatal vitamins. Start eating small, frequent meals if nausea occurs.",
        askProvider: "Ask about safe medications for nausea and when to schedule your first ultrasound.",
        checklist: ["Confirm pregnancy with test", "Call healthcare provider", "Continue vitamins", "Rest when tired"],
        alertSigns: ["Severe nausea/vomiting", "Dizziness/fainting", "Heavy bleeding"],
        milestone: "Baby's heart begins to form and start beating"
    },
    3: {
        title: "Neural Tube Development",
        babySize: "Lentil",
        babyGrowth: "The neural tube is forming, which becomes brain and spinal cord. Tiny heart is beating and circulating blood.",
        motherChanges: "Fatigue is common as your body works hard. Morning sickness may begin. Breasts may feel tender.",
        careFocus: "Stay hydrated. Eat protein-rich foods. Get extra rest when possible.",
        askProvider: "Discuss your medical history and any genetic testing options.",
        checklist: ["Schedule first prenatal visit", "Stay hydrated", "Eat small meals", "Get extra sleep"],
        alertSigns: ["Inability to keep fluids down", "Severe fatigue", "Abnormal discharge"],
        milestone: "Baby's heart is now beating (about 100-160 bpm)"
    },
    4: {
        title: "Organ Systems Begin",
        babySize: "Blueberry",
        babyGrowth: "Major organs are forming: brain, heart, liver, kidneys. Limb buds appear where arms and legs will grow.",
        motherChanges: "Morning sickness may peak. You might feel emotional and have food aversions or cravings.",
        careFocus: "Continue prenatal care. Eat bland foods if nauseous. Stay active with gentle exercise.",
        askProvider: "Ask about weight gain goals and safe exercises for pregnancy.",
        checklist: ["Attend first prenatal appointment", "Discuss genetic testing", "Stay active", "Manage nausea"],
        alertSigns: ["Dehydration", "Rapid weight loss", "Persistent vomiting"],
        milestone: "All major organs begin to form"
    },
    5: {
        title: "Facial Features Form",
        babySize: "Apple seed",
        babyGrowth: "Baby's face is taking shape with eyes, nose, and mouth. Webbed fingers and toes are forming.",
        motherChanges: "Morning sickness may continue. You might notice clothes fitting tighter. Some women feel more emotional.",
        careFocus: "Focus on nutrition, especially protein and calcium. Continue gentle exercise.",
        askProvider: "Ask about prenatal screening tests and when they should be done.",
        checklist: ["Consider genetic screening", "Eat balanced meals", "Stay hydrated", "Get regular exercise"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's facial features become recognizable"
    },
    6: {
        title: "Rapid Growth Spurt",
        babySize: "Sweet pea",
        babyGrowth: "Baby is growing quickly. Intestines are developing and will soon move into the abdominal cavity.",
        motherChanges: "Morning sickness may start improving. You might feel less tired. Some women notice a slight weight gain.",
        careFocus: "Focus on iron-rich foods. Continue prenatal vitamins and regular exercise.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Eat iron-rich foods", "Continue exercise", "Track symptoms", "Stay hydrated"],
        alertSigns: ["Severe cramping", "Bleeding", "Dizziness"],
        milestone: "Baby's intestines begin to develop"
    },
    7: {
        title: "Facial Features Develop",
        babySize: "Blueberry",
        babyGrowth: "Baby's facial features are more defined. Eyelids and toes are forming. Baby can make tiny movements.",
        motherChanges: "Morning sickness may be decreasing. You might notice your abdomen is slightly firmer.",
        careFocus: "Continue healthy eating and exercise. Start pelvic floor exercises.",
        askProvider: "Ask about upcoming ultrasounds and what to expect.",
        checklist: ["Start pelvic floor exercises", "Eat calcium-rich foods", "Stay active", "Rest when needed"],
        alertSigns: ["Severe nausea", "Abdominal pain", "Bleeding"],
        milestone: "Baby can make small movements"
    },
    8: {
        title: "Fingers & Toes Form",
        babySize: "Kidney bean",
        babyGrowth: "Webbed fingers and toes are separating. Baby can now make small, jerky movements.",
        motherChanges: "You might feel less nauseous. Some women notice a slight weight gain. Breasts may continue to feel tender.",
        careFocus: "Continue prenatal vitamins. Focus on balanced nutrition and hydration.",
        askProvider: "Ask about your first ultrasound appointment and what to expect.",
        checklist: ["Schedule first ultrasound", "Continue vitamins", "Eat balanced meals", "Stay active"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "Fever"],
        milestone: "Fingers and toes are fully formed and separated"
    },
    9: {
        title: "Baby Can Hear Sounds",
        babySize: "Grape",
        babyGrowth: "All essential organs are formed. Baby's ears are developing and can hear sounds like your heartbeat.",
        motherChanges: "Morning sickness should be improving. You might feel more energetic. Some women notice a small baby bump.",
        careFocus: "Continue healthy eating. Start talking and singing to your baby to help with bonding.",
        askProvider: "Ask about hearing development and when baby can respond to sounds.",
        checklist: ["Talk to your baby", "Continue exercise", "Eat protein-rich foods", "Stay hydrated"],
        alertSigns: ["Severe nausea", "Abdominal pain", "Bleeding"],
        milestone: "Baby can hear sounds from outside the womb"
    },
    10: {
        title: "Fingernails & Toenails Form",
        babySize: "Strawberry",
        babyGrowth: "Baby's fingernails and toenails are beginning to form. Baby can now make purposeful movements.",
        motherChanges: "You might feel more comfortable as morning sickness decreases. Some women notice weight gain.",
        careFocus: "Continue prenatal care. Focus on nutrition and stay active.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Continue prenatal vitamins", "Eat balanced meals", "Stay active", "Track weight gain"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's fingernails and toenails begin to form"
    },
    11: {
        title: "Baby Can Suck Thumb",
        babySize: "Fig",
        babyGrowth: "Baby can now suck their thumb and has distinct fingerprints. Baby is very active.",
        motherChanges: "You might feel more energetic. Some women notice a small baby bump. Morning sickness should be minimal.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about upcoming screenings and when to schedule them.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Bleeding", "Fever"],
        milestone: "Baby can suck thumb and has fingerprints"
    },
    12: {
        title: "End of First Trimester",
        babySize: "Lime",
        babyGrowth: "Baby is now fully formed with all organs and systems. Baby will mainly grow and mature from now on.",
        motherChanges: "Risk of miscarriage drops significantly. You might feel more energetic. Some women notice a visible baby bump.",
        careFocus: "Celebrate reaching this milestone! Continue prenatal care and healthy habits.",
        askProvider: "Ask about second trimester screenings and when to schedule them.",
        checklist: ["Schedule second trimester screening", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "All major organs are formed and functional"
    },
    13: {
        title: "Beginning of Second Trimester",
        babySize: "Pea pod",
        babyGrowth: "Baby's head is about half the body length. Baby can now make more coordinated movements.",
        motherChanges: "Many women feel their best during this trimester. Morning sickness is usually gone. Energy levels increase.",
        careFocus: "Enjoy your energy boost! Continue healthy eating and exercise.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Continue prenatal vitamins", "Eat balanced meals", "Stay active", "Track weight gain"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's movements become more coordinated"
    },
    14: {
        title: "Baby Can Make Faces",
        babySize: "Lemon",
        babyGrowth: "Baby can make facial expressions like squinting and frowning. Baby can also swallow and urinate.",
        motherChanges: "You might feel the baby move for the first time (quickening). Some women notice weight gain.",
        careFocus: "Continue healthy eating and exercise. Start thinking about maternity clothes.",
        askProvider: "Ask about when you should feel movement and what's normal.",
        checklist: ["Consider maternity clothes", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can make facial expressions"
    },
    15: {
        title: "Baby Can Hear Your Voice",
        babySize: "Apple",
        babyGrowth: "Baby's hearing is well developed. Baby can hear your voice and will recognize it after birth.",
        motherChanges: "You might feel more movement. Some women notice a visible baby bump. Energy levels are usually high.",
        careFocus: "Continue healthy eating and exercise. Talk and sing to your baby.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Talk to your baby", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can hear and recognize your voice"
    },
    16: {
        title: "Baby Can Make Breathing Movements",
        babySize: "Avocado",
        babyGrowth: "Baby is practicing breathing movements by moving amniotic fluid in and out of lungs. Baby can also yawn.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
    ...
    17: {
        title: "Baby Can Blink",
        babySize: "Turnip",
        babyGrowth: "Baby can now blink and has more defined features. Baby is also developing fat stores.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about maternity leave.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider maternity leave", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can blink and has more defined features"
    },
    18: {
        title: "Baby Can Hear Your Heartbeat",
        babySize: "Bell pepper",
        babyGrowth: "Baby's hearing is very well developed. Baby can hear your heartbeat and other internal sounds.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can hear internal sounds like your heartbeat"
    },
    19: {
        title: "Baby Can Make Purposeful Movements",
        babySize: "Tomato",
        babyGrowth: "Baby is making more purposeful movements and can now grasp things. Baby is also developing a protective coating.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can make purposeful movements and grasp"
    },
    20: {
        title: "Halfway Point - Baby Can Hear Sounds",
        babySize: "Banana",
        babyGrowth: "Baby can hear sounds from outside the womb and will respond to familiar voices. Baby is also developing sleep patterns.",
        motherChanges: "You're halfway there! You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Celebrate this milestone! Continue healthy eating and exercise.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Celebrate halfway point", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can hear sounds from outside and has sleep patterns"
    },
    21: {
        title: "Baby Can Taste",
        babySize: "Carrot",
        babyGrowth: "Baby's taste buds are developing and baby can now taste the amniotic fluid. Baby is also gaining weight.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can taste and is gaining weight"
    },
    22: {
        title: "Baby Has Eyebrows and Eyelashes",
        babySize: "Spaghetti squash",
        babyGrowth: "Baby has eyebrows and eyelashes. Baby's eyes can now move from side to side.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby has eyebrows and eyelashes"
    },
    23: {
        title: "Baby Can Respond to Touch",
        babySize: "Large mango",
        babyGrowth: "Baby can now respond to touch and pressure. Baby is also developing a more regular sleep pattern.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can respond to touch and has regular sleep patterns"
    },
    24: {
        title: "Baby Has Hair and Nails",
        babySize: "Ear of corn",
        babyGrowth: "Baby has hair on their head and fingernails are fully formed. Baby is also gaining weight rapidly.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby has hair and fully formed nails"
    },
    25: {
        title: "Baby Can Make Breathing Movements",
        babySize: "Rutabaga",
        babyGrowth: "Baby is practicing breathing movements and developing more complex brain connections.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby practices breathing and develops complex brain connections"
    },
    26: {
        title: "Baby Can Open Eyes",
        babySize: "Scallion",
        babyGrowth: "Baby's eyes are now open and baby can see light filtering through the womb. Baby is also gaining weight.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's eyes are open and can see light"
    },
    27: {
        title: "End of Second Trimester",
        babySize: "Cauliflower",
        babyGrowth: "Baby's lungs are developing rapidly. Baby is also gaining weight and developing more fat.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's lungs are developing rapidly and gaining weight"
    },
    28: {
        title: "Beginning of Third Trimester",
        babySize: "Eggplant",
        babyGrowth: "Baby's brain is developing rapidly. Baby can now dream and has more regular sleep patterns.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can dream and has regular sleep patterns"
    },
    29: {
        title: "Baby Can Kick Hard",
        babySize: "Butternut squash",
        babyGrowth: "Baby's kicks are getting stronger and more frequent. Baby is also gaining weight rapidly.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's kicks are stronger and more frequent"
    },
    30: {
        title: "Baby Can Grasp",
        babySize: "Cabbage",
        babyGrowth: "Baby can now grasp things and has more developed senses. Baby is also gaining weight rapidly.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can grasp and has more developed senses"
    },
    31: {
        title: "Baby Can See Light",
        babySize: "Coconut",
        babyGrowth: "Baby can now see light and has more developed senses. Baby is also gaining weight rapidly.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby can see light and has more developed senses"
    },
    32: {
        title: "Baby Has Fully Developed Senses",
        babySize: "Jicama",
        babyGrowth: "Baby's senses are fully developed. Baby can now taste, smell, hear, see, and touch.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby has fully developed senses"
    },
    33: {
        title: "Baby Can Practice Breathing",
        babySize: "Pineapple",
        babyGrowth: "Baby is practicing breathing movements and developing more complex brain connections.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
    ...
    34: {
        title: "Baby Has Strong Immune System",
        babySize: "Cantaloupe",
        babyGrowth: "Baby's immune system is developing and baby is gaining antibodies from you.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's immune system is developing and gaining antibodies"
    },
    35: {
        title: "Baby Has Fully Developed Lungs",
        babySize: "Honeydew",
        babyGrowth: "Baby's lungs are fully developed and baby is preparing for birth. Baby is also gaining weight rapidly.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby's lungs are fully developed and preparing for birth"
    },
    36: {
        title: "Baby Is Getting Ready for Birth",
        babySize: "Head of lettuce",
        babyGrowth: "Baby is getting into position for birth and is gaining weight rapidly. Baby's head may engage in pelvis.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby is getting into birth position and gaining weight"
    },
    37: {
        title: "Baby Is Full Term",
        babySize: "Winter melon",
        babyGrowth: "Baby is now full term and ready for birth. Baby's organs are fully developed.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby is full term and ready for birth"
    },
    38: {
        title: "Baby Is Getting Ready for Birth",
        babySize: "Pumpkin",
        babyGrowth: "Baby is getting ready for birth and is gaining weight rapidly. Baby's head may engage in pelvis.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby is getting ready for birth and gaining weight"
    },
    39: {
        title: "Baby Is Almost Ready",
        babySize: "Watermelon",
        babyGrowth: "Baby is almost ready for birth and is gaining weight rapidly. Baby's head may engage in pelvis.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby is almost ready for birth and gaining weight"
    },
    40: {
        title: "Baby Is Ready for Birth",
        babySize: "Small watermelon",
        babyGrowth: "Baby is ready for birth and has fully developed organs. Baby is also gaining weight rapidly.",
        motherChanges: "You might feel more movement. Some women notice weight gain and a visible baby bump.",
        careFocus: "Continue healthy eating and exercise. Start thinking about birth plans.",
        askProvider: "Ask about your weight gain and any concerns about symptoms.",
        checklist: ["Consider birth plans", "Continue exercise", "Eat balanced meals", "Stay hydrated"],
        alertSigns: ["Severe abdominal pain", "Heavy bleeding", "High fever"],
        milestone: "Baby is ready for birth with fully developed organs"
    }
};

// Pregnancy Week-by-Week Tracker Functions
let currentPregnancyWeek = null;

function initializePregnancyWeeks() {
    const firstTrimester = document.getElementById('firstTrimester');
    const secondTrimester = document.getElementById('secondTrimester');
    const thirdTrimester = document.getElementById('thirdTrimester');
    
    // First Trimester (Weeks 1-13)
    for (let week = 1; week <= 13; week++) {
        const weekCard = createWeekCard(week);
        firstTrimester.appendChild(weekCard);
    }
    
    // Second Trimester (Weeks 14-27)
    for (let week = 14; week <= 27; week++) {
        const weekCard = createWeekCard(week);
        secondTrimester.appendChild(weekCard);
    }
    
    // Third Trimester (Weeks 28-42)
    for (let week = 28; week <= 42; week++) {
        const weekCard = createWeekCard(week);
        thirdTrimester.appendChild(weekCard);
    }
}
...
function createWeekCard(week) {
    ...
    div.onclick = () => showWeekDetail(week);
    div.innerHTML = `
        <div class="week-number">Week ${week}</div>
        <div class="week-size">${weekData.babySize}</div>
...
function showWeekDetail(week) {
    ...
    // Update detail panel
    document.getElementById('weekStageLabel').textContent = 
        week <= 13 ? 'First Trimester' : 
        week <= 27 ? 'Second Trimester' : 'Third Trimester';
    
    document.getElementById('weekDetailTitle').textContent = `Week ${week}: ${weekData.title}`;
    document.getElementById('weekBabySize').textContent = weekData.babySize;
    document.getElementById('weekMilestoneText').textContent = `Milestone: ${weekData.milestone}`;
    
    document.getElementById('weekBabyGrowth').textContent = weekData.babyGrowth;
    document.getElementById('weekMotherChanges').textContent = weekData.motherChanges;
    document.getElementById('weekCareFocus').textContent = weekData.careFocus;
    document.getElementById('weekAskProvider').textContent = weekData.askProvider;
    
    // Update checklist
    const checklistEl = document.getElementById('weekChecklist');
    checklistEl.innerHTML = weekData.checklist.map(item => `<li>${item}</li>`).join('');
    
    // Update alert signs
    const alertSignsEl = document.getElementById('weekAlertSigns');
    alertSignsEl.innerHTML = weekData.alertSigns.map(sign => `<li>${sign}</li>`).join('');
    
    // Highlight selected week
    document.querySelectorAll('.week-card').forEach(card => {
        card.classList.remove('selected');
    });
    event.target.closest('.week-card').classList.add('selected');
    
    // Scroll to detail panel
    document.getElementById('weekDetailPanel').scrollIntoView({ behavior: 'smooth' });
}
...
// Initialize pregnancy weeks when page loads
document.addEventListener('DOMContentLoaded', function() {
    // Check if we're on pregnancy page
    if (document.getElementById('firstTrimester')) {
        initializePregnancyWeeks();
    }
});
