// Frontend API Service for Mamasafe Application
class ApiService {
    constructor() {
    ...
    // Chatbot methods: backend Llama/RAG pipeline first, compatibility endpoints after.
    async sendChatMessage(message, userContext = {}, chatHistory = []) {
        const payload = {
            message,
            week: userContext.pregnancyWeek || userContext.week || '',
            pregnancyWeek: userContext.pregnancyWeek || userContext.week || '',
            symptoms: userContext.healthConcerns || userContext.symptoms || '',
            userContext,
            chatHistory
        };

        try {
            return await this.post('/ai/ask', payload);
        } catch (error) {
            console.warn('AI pipeline endpoint failed, falling back to /chat:', error.message);
            try {
                return await this.post('/chat', payload);
            } catch {
                return this.post('/mamasafe-chat', payload);
            }
        }
    }
