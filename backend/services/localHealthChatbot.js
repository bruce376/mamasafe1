...
const healthKnowledgeBase = {
  pregnancy: {
    ...
    babyCare: {
      "newborn sleep": {
        response: "Newborn sleep patterns and tips:\n\n**Normal Sleep Patterns:**\n• Newborns sleep 16-18 hours per day (usually in 2-4 hour stretches)\n• No day/night distinction initially\n• Sleep cycles are shorter (50-60 minutes)\n• Often make noises and move during sleep\n\n**Safe Sleep Practices:**\n• Always place baby on back to sleep\n• Use firm, flat sleep surface\n• No loose bedding, pillows, or toys in crib\n• Room temperature should be comfortable (68-72°F)\n• Consider swaddling for the first few months\n• Share room but not bed for first 6 months\n\n**Sleep Environment Tips:**\n• Use white noise or soft music\n• Keep room dark for nighttime sleep\n• Establish consistent bedtime routine\n• Avoid overstimulation before bedtime\n\n**When to Call Your Doctor:**\n• Baby seems unusually lethargic or difficult to wake\n• Breathing irregularities during sleep\n• Poor weight gain due to sleep issues\n\n",
        keywords: ["sleep", "newborn", "baby sleep", "sleep patterns"]
      },
      "feeding": {
        response: "Newborn feeding guidelines:\n\n**Breastfeeding:**\n**Frequency:**\n• Feed on demand (8-12 times per day)\n• Watch for hunger cues (rooting, hand-to-mouth, sucking)\n• Newborn stomach is tiny (size of cherry)\n\n**Signs of Effective Feeding:**\n• Baby swallows audibly\n• Baby has 6+ wet diapers per day\n• 3+ bowel movements daily after day 3\n• Baby seems satisfied after feeding\n• Steady weight gain\n\n**Bottle Feeding:**\n• Use formula appropriate for age\n• Follow preparation instructions exactly\n• Hold baby during feeding (never prop bottle)\n• Burp baby frequently\n• Discard unused formula after feeding\n\n**When to Call Your Doctor:**\n• Fewer than 6 wet diapers in 24 hours\n• Signs of dehydration (no tears, dry mouth)\n• Poor weight gain or weight loss\n• Yellowing of skin or eyes (jaundice)\n• Difficulty latching or refusing to feed\n\n",
        keywords: ["feeding", "breastfeeding", "bottle feeding", "nursing"]
      }
    },
    emergencies: {
      "bleeding": {
...
function getEmergencyResponse(keyword) {
  if (keyword.includes('bleeding') || keyword.includes('blood') || keyword.includes('spotting')) {
    return healthKnowledgeBase.emergencies.bleeding.response;
  } else if (keyword.includes('pain') || keyword.includes('cramping')) {
    return healthKnowledgeBase.emergencies.severePain.response;
  } else {
...
function getGeneralHealthResponse(message, userContext) {
  ...
  if (greetings.some(greeting => lowerMessage === greeting || lowerMessage.startsWith(greeting + ' '))) {
    ...
    if (userContext.pregnancyWeek) {
      ...
      contextPrefix = `Hello! I see you have a ${userContext.babyAge}-month-old baby. `;
    } else if (userContext.toddlerAge) {
      contextPrefix = `Hello! I see you have a ${userContext.toddlerAge}-year-old toddler. `;
    } else {
      contextPrefix = `Hello! `;
    }
    
    return `${contextPrefix}I'm here to help you with any questions or concerns you might have. Feel free to ask me about pregnancy symptoms, nutrition, baby care, or any other topics. How can I assist you today?`;
  }
  ...
  if (userContext.pregnancyWeek) {
    ...
    contextPrefix = `Since you have a ${userContext.babyAge}-month-old baby, `;
  } else if (userContext.toddlerAge) {
    contextPrefix = `Since you have a ${userContext.toddlerAge}-year-old toddler, `;
  }
...
Would you like me to help you with information about common pregnancy symptoms, nutrition guidelines, or baby care topics that I do have detailed information about?`;
}
...
function generateLocalHealthSuggestions(userContext = {}) {
  ...
  if (userContext.babyAge) {
    const age = parseInt(userContext.babyAge);
    if (age <= 3) {
      return [
